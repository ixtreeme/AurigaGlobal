...
void CMapOutdoor::CreateTerrainPatchProxyList()
{
	...
	m_pTerrainPatchProxyList = std::make_unique<CTerrainPatchProxy[]>(m_wPatchCount * m_wPatchCount);
...
bool CMapOutdoor::GetPickingPointWithRay(const CRay & rRay, D3DXVECTOR3 * v3IntersectPt)
{
	...
	{
		auto obj = v3ObjectPick - v3Start;
		auto ter = v3TerrainPick - v3Start;

		if (D3DXVec3Length(&obj) >= D3DXVec3Length(&ter))
...
void CMapOutdoor::GetHeightMap(const uint8_t& c_rucTerrainNum, WORD ** pwHeightMap)
{
...
bool CMapOutdoor::GetWaterHeight(int iX, int iY, int32_t* plWaterHeight)
{
...
bool CMapOutdoor::GetTerrainNumFromCoord(WORD wCoordX, WORD wCoordY, uint8_t* pbyTerrainNum)
{
...
void CMapOutdoor::VisibleMarkedArea()
{
	...
	std::map<int, uint8_t*>::iterator itorTerrain = kMap_pbyMarkBuf.begin();
...
void CMapOutdoor::ConvertToMapCoords(float fx, float fy, int *iCellX, int *iCellY, uint8_t* pucSubCellX, uint8_t* pucSubCellY, WORD * pwTerrainNumX, WORD * pwTerrainNumY)
{
	if ( fy < 0 )
		fy = -fy;
