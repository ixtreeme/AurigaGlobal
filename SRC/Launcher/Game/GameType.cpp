...
bool NRaceData::THitData::Load(CTextFileLoader & rTextFileLoader)
{
	...
	if (rTextFileLoader.GetTokenVector("hitposition",&tv))
	{
		...
		while(it!=tv->end())
		{
			...
			time = std::stof(*it++);
			hp.v3LastPosition.x = std::stof(*it++);
			hp.v3LastPosition.y = std::stof(*it++);
			hp.v3LastPosition.z = std::stof(*it++);
			hp.v3Position.x = std::stof(*it++);
			hp.v3Position.y = std::stof(*it++);
			hp.v3Position.z = std::stof(*it++);
