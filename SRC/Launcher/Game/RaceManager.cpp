...
void __GetRaceResourcePathes(unsigned race, std::vector <std::string> & vec_stPathes)
{
	...
	else if (__IsNPCRace(race))
	{
		if (race >= 30000)
		{
			...
			{
				vec_stPathes.emplace_back("d:/ymir work/npc_pet/"); //FOLDER PATCH PET SYSTEM DIRECTORY
			}
#endif
...
{
	for (auto& [key, raceData] : m_RaceDataMap)
	{
		CRaceData::Delete(raceData);
	}
