...
bool CPropertyManager::Initialize(const char * c_pszPackFileName)
{
	if (c_pszPackFileName)
	{
		...
		uint8_t* pbListData;
		if (CEterPackManager::Instance().Get(kListFile, "property/list", (const void**)&pbListData))
		{
		...
		uint8_t* pbPropertyXML;
		if (CEterPackManager::Instance().Get(kPropertyXML, c_pszPackFileName, (const void**)&pbPropertyXML))
		{
