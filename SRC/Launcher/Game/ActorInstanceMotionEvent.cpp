...
#endif
{
	...
	switch (c_pData->iType)
	{
		...
		case CRaceMotionData::MOTION_EVENT_TYPE_SCREEN_WAVING:
			CGameEventManager::Instance().ProcessEventScreenWaving(this, dynamic_cast<const CRaceMotionData::TScreenWavingEventData*>(c_pData));
			break;
...
#endif
{
	...
	auto c_pEffectData = dynamic_cast<const CRaceMotionData::TMotionEffectEventData*>(c_pData);
	...
	{
#ifdef ENABLE_SKILL_COLOR_SYSTEM
		AttachEffectByID(0, nullptr, c_pEffectData->dwEffectIndex, &c_pEffectData->v3EffectPosition, dwSkillColor);
#else
...
#endif
{
	...
	const auto c_pEffectToTargetData = dynamic_cast<const CRaceMotionData::TMotionEffectToTargetEventData*>(c_pData);
	...
	else
	{
		...
		{
			const auto pTargetInstance = dynamic_cast<CActorInstance*>(m_kFlyTarget.GetFlyTarget());
...
#ifdef ENABLE_SKILL_COLOR_SYSTEM
			pTargetInstance->AttachEffectByID(0, nullptr, c_pEffectToTargetData->dwEffectIndex, &v3Position, dwSkillColor);
#else
...
void CActorInstance::ProcessMotionEventSpecialAttacking(int iMotionEventIndex, const CRaceMotionData::TMotionEventData * c_pData)
{
	...
	const auto c_pAttackingData = dynamic_cast<const CRaceMotionData::TMotionAttackingEventData*>(c_pData);

	const float fRadian = XMConvertToRadians(270.0f + 360.0f - GetRotation());
...
void CActorInstance::ProcessMotionEventSound(const CRaceMotionData::TMotionEventData * c_pData)
{
	...
	const auto c_pSoundData = dynamic_cast<const CRaceMotionData::TMotionSoundEventData*>(c_pData);
...
#endif
{
	...
	auto c_pFlyData = dynamic_cast<const CRaceMotionData::TMotionFlyEventData*>(c_pData);
	...
	if (m_kFlyTarget.IsValidTarget())
	{
		...
		if (c_pFlyData->isAttaching)
		{
			...
			if (uint32_t dwPartIndex = 0; FindBoneIndex(dwPartIndex, c_pFlyData->strAttachingBoneName.c_str(), &iBoneIndex))
