...
bool CRaceMotionData::GetMotionEventDataPointer(uint8_t byIndex, const TMotionEventData ** c_ppData) const
{
...
bool CRaceMotionData::GetMotionAttackingEventDataPointer(uint8_t byIndex, const TMotionAttackingEventData ** c_ppData) const
{
	if (byIndex >= m_MotionEventDataVector.size())
		return FALSE;

	const TMotionEventData * pData = m_MotionEventDataVector[byIndex];
	const auto pAttackingEvent = dynamic_cast<const TMotionAttackingEventData*>(pData);
...
bool CRaceMotionData::LoadMotionData(const char * c_szFileName)
{
	...
	if (rkTextFileLoader.GetTokenVector("accumulation", &pTokenVector))
	{
		...
		TPixelPosition pos(std::stof(pTokenVector->at(0)),
						   std::stof(pTokenVector->at(1)),
						   std::stof(pTokenVector->at(2)));
