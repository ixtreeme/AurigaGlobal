...
void CTerrainPatch::BuildWaterVertexBuffer(SWaterVertex* akSrcVertex, UINT uWaterVertexCount)
{
	...
	if (!rkVB.Create((int)uWaterVertexCount, D3DFVF_XYZ | D3DFVF_DIFFUSE, D3DUSAGE_WRITEONLY, D3DPOOL_MANAGED))
		return;
...
void CTerrainPatch::SoftwareTransformPatch_UpdateTerrainLighting(uint32_t dwVersion, const D3DLIGHT9& c_rkLight, const D3DMATERIAL9& c_rkMtrl)
{
...
void CTerrainPatchProxy::SoftwareTransformPatch_UpdateTerrainLighting(uint32_t dwVersion, const D3DLIGHT9& c_rkLight, const D3DMATERIAL9& c_rkMtrl)
{
	if (m_pTerrainPatch)
