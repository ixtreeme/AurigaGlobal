...
void CMapOutdoor::RenderWater()
{
	...
	//STATEMANAGER.SaveTextureStageState(0, D3DTSS_TEXCOORDINDEX, D3DTSS_TCI_CAMERASPACENORMAL);
	...
	s_fWaterHeightCurrent = s_fWaterHeightBegin + (s_fWaterHeightEnd - s_fWaterHeightBegin) * ((CTimer::Instance().GetCurrentMillisecond() - s_dwLastHeightChangeTime) / (float) s_dwBlendtime);
	...
	//STATEMANAGER.SetRenderState(D3DRS_ALPHABLENDENABLE, FALSE); // itt
