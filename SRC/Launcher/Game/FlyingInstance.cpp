...
void CFlyingInstance::AdjustDirectionForHoming(const D3DXVECTOR3 & v3TargetPosition)
{
	...
	auto ix = vVel - vTargetDir;
	if (D3DXVec3LengthSq(&ix) < 0.001f)
		return;
...
void CFlyingInstance::UpdateAttachInstance()
{
	...
	D3DXQuaternionRotationYawPitchRoll(&q,
		XMConvertToRadians(m_pData->m_v3AngVel.y)*CTimer::Instance().GetElapsedSecond(),
		XMConvertToRadians(m_pData->m_v3AngVel.x)*CTimer::Instance().GetElapsedSecond(),
		XMConvertToRadians(m_pData->m_v3AngVel.z)*CTimer::Instance().GetElapsedSecond());
...
void CFlyingInstance::__SetDataPointer(CFlyingData* pData, const D3DXVECTOR3& v3StartPosition)
{
	...
	D3DXQuaternionRotationYawPitchRoll(&m_qRot, XMConvertToRadians(pData->m_fRollAngle - 90.0f), 0.0f, XMConvertToRadians(pData->m_fConeAngle));
	if (pData->m_bSpreading)
	{
		D3DXQUATERNION q1, q2;

		auto ix_vec = D3DXVECTOR3(0.0f, 0.0f, 1.0f);
		auto ix_vec2 = D3DXVECTOR3(0.0f, -1.0f, 0.0f);
		D3DXQuaternionRotationAxis(&q2, &ix_vec, (frandom(-3.141592f / 3, +3.141592f / 3) + frandom(-3.141592f / 3, +3.141592f / 3)) / 2);
		D3DXQuaternionRotationAxis(&q1, &ix_vec2, frandom(0, 2 * 3.1415926535897931f));
