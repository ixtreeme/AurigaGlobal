...
void CMapOutdoor::SetInverseViewAndDynamicShaodwMatrices()
{
	...
	D3DXMatrixLookAtRH(&m_matLightView, &v3LightEye, &v3Target, &ix);
...
struct CMapOutdoor_LessThingInstancePtrRenderOrder
{
	bool operator() (CGraphicThingInstance* pkLeft, CGraphicThingInstance* pkRight)
	{
		...
		auto vec_l = D3DXVECTOR3(c_rv3CameraPos - c_v3LeftPos);
		auto vec_r = D3DXVECTOR3(c_rv3CameraPos - c_v3RightPos);

		return D3DXVec3LengthSq(&vec_l) < D3DXVec3LengthSq(&vec_r);
	}
};
...
void CMapOutdoor::SelectIndexBuffer(uint8_t byLODLevel, WORD * pwPrimitiveCount, D3DPRIMITIVETYPE * pePrimitiveType)
{
...
struct FPatchNumMatch
{
	...
	explicit FPatchNumMatch(int32_t lPatchNum)
	{
		m_lPatchNumToCheck = lPatchNum;
	}
	bool operator() (std::pair<int32_t, uint8_t> aPair) const
	{
...
void CMapOutdoor::DrawWireFrame(int32_t patchnum, WORD wPrimitiveCount, D3DPRIMITIVETYPE ePrimitiveType)
{
