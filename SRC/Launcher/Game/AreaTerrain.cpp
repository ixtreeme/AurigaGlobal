...
bool CTerrain::LoadShadowMap(const char* c_pszFileName)
{
	...
	uint32_t dwShadowMapSize = sizeof(WORD) * 256 * 256;
...
bool CTerrain::isAttrOn(WORD wCoordX, WORD wCoordY, uint8_t byAttrFlag)
{
...
uint8_t CTerrain::GetAttr(WORD wCoordX, WORD wCoordY)
{
	if (wCoordX >= ATTRMAP_XSIZE || wCoordY >= ATTRMAP_YSIZE)
	{
...
bool CTerrain::GetWaterHeight(WORD wCoordX, WORD wCoordY, int32_t* plWaterHeight)
{
...
void CTerrain::RAW_CountTiles()
{
	...
	{
		int32_t lPatchIndexY = min(max((y - 1) / PATCH_TILE_YSIZE, 0), PATCH_YCOUNT - 1);
		for (int32_t x = 0; x < TILEMAP_RAW_XSIZE; ++x)
		{
			int32_t lPatchIndexX = min(max((x - 1) / (PATCH_TILE_XSIZE), 0), PATCH_XCOUNT - 1);
...
LPDIRECT3DTEXTURE9 CTerrain::AddTexture32(uint8_t byImageNum, uint8_t* pbyImage, int32_t lTextureWidth, int32_t lTextureHeight)
{
	...
	hr = ms_lpd3dDevice->CreateTexture(uiNewWidth, uiNewHeight, 5, 0, format, D3DPOOL_MANAGED, &pkTex, nullptr);
	if (FAILED(hr))
	{
