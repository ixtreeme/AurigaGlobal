...
bool CMapOutdoor::Load(float x, float y, float z)
{
	...
	CEterPackManager& rkPackMgr=CEterPackManager::Instance();
	{
		...
		static std::string s_strOldPathName=c_rstrNewPathName;
	}
...
#endif
{
	...
	snprintf(szAreaPathName, sizeof(szAreaPathName), "%s\\%06u\\", GetMapDataDirectory().c_str(), ulID);
...
#endif
{
	...
	snprintf(szRawHeightFieldname, sizeof(szRawHeightFieldname), "%s\\%06u\\height.raw", GetMapDataDirectory().c_str(), ulID);
	snprintf(szSplatName, sizeof(szSplatName), "%s\\%06u\\tile.raw", GetMapDataDirectory().c_str(), ulID);
	snprintf(szAttrMapName, sizeof(szAttrMapName), "%s\\%06u\\attr.atr", GetMapDataDirectory().c_str(), ulID);
	snprintf(szWaterMapName, sizeof(szWaterMapName), "%s\\%06u\\water.wtr", GetMapDataDirectory().c_str(), ulID);
	snprintf(szShadowTexName, sizeof(szShadowTexName), "%s\\%06u\\shadowmap.dds", GetMapDataDirectory().c_str(), ulID);
	snprintf(szShadowMapName, sizeof(szShadowMapName), "%s\\%06u\\shadowmap.raw", GetMapDataDirectory().c_str(), ulID);
	snprintf(szMiniMapTexName, sizeof(szMiniMapTexName), "%s\\%06u\\minimap.dds", GetMapDataDirectory().c_str(), ulID);
...
bool CMapOutdoor::LoadSetting(const char * c_szFileName)
{
	...
	if (!stTokenVectorMap.contains("watertype"))
	{
		TraceError("MapOutdoor::LoadSetting(c_szFileName=%s) - FIND 'watertype' - FAILED", c_szFileName);
		return false;
	}

	uint8_t WaterType = atoi(stTokenVectorMap["watertype"][0].c_str());
	if (WaterType < 0 || WaterType > WATERTYPE_COUNT)
	{
	...
	m_lViewRadius = std::stoi(c_rstrViewRadius);
	...
	m_fHeightScale = std::stof(c_rstrHeightScale);
	...
	if (stTokenVectorMap.contains("baseposition"))
	{
		...
		SetBaseXY(std::stoul(c_rstrMapBaseX), std::stoul(c_rstrMapBaseY));
	}
...
bool CMapOutdoor::LoadMonsterAreaInfo()
{
	...
	for (uint32_t i = 0; i < textFileLoader.GetLineCount(); ++i)
	{
		...
		if (0 == stTokenVector[0].compare("m") || 0 == stTokenVector[0].compare("g"))
		{
			...
			int32_t lOriginX = atol(c_rstrOriginX.c_str());
			int32_t lOriginY = atol(c_rstrOriginY.c_str());
			int32_t lSizeX = atol(c_rstrSizeX.c_str());
			int32_t lSizeY = atol(c_rstrSizeY.c_str());
			int32_t lZ = atol(c_rstrZ.c_str());
			CMonsterAreaInfo::EMonsterDir eMonsterDir = (CMonsterAreaInfo::EMonsterDir)atoi(c_rstrDir.c_str());
			int32_t lTime = atol(c_rstrTime.c_str());
			int32_t lPercent = atol(c_rstrPercent.c_str());
			uint32_t dwMonsterCount = (uint32_t)atoi(c_rstrCount.c_str());
			uint32_t dwMonsterVID = (uint32_t)atoi(c_rstrVID.c_str());
