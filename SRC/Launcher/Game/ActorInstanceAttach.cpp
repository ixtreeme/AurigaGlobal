...
uint32_t CActorInstance::AttachSmokeEffect(uint32_t eSmoke)
{
	...
	const uint32_t dwSmokeEffectID = m_pkCurRaceData->GetSmokeEffectID(eSmoke);
...
bool CActorInstance::__IsLeftHandWeapon(uint32_t type)
{
	...
	{
		// Mounted assassin (ninja): keep dagger only in right hand.
		if (__IsMountingHorse())
		{
			...
			const uint32_t dwRace = GetRace();
			if (dwRace == MAIN_RACE_ASSASSIN_W || dwRace == MAIN_RACE_ASSASSIN_M)
				return false;
		}

		return true;
	}
	else if (CItemData::WEAPON_FAN == type && __IsMountingHorse())
		return true;
...
bool CActorInstance::GetAttachingBoneName(uint32_t dwPartIndex, const char** c_pszBoneName) const
{
...
void CActorInstance::AttachWeapon(uint32_t dwParentPartIndex, uint32_t dwPartIndex, CItemData* pItemData)
{
	...
	if (CGrannyLODController* pLODController = m_LODControllerVector[dwPartIndex])
	{
		if (CGrannyModelInstance* pWeaponModelInstance = pLODController->GetModelInstance())
		{
			...
			if (pItemGraphicThing)
			{
				if (auto pItemMotion = pItemGraphicThing->GetMotionPointer(0))
				{
...
uint32_t CActorInstance::AttachEffectByName(uint32_t dwParentPartIndex, const char* c_pszBoneName, const char* c_pszEffectName)
{
	...
	const uint32_t dwCRC = GetCaseCRC32(str.c_str(), str.length());

	return AttachEffectByID(dwParentPartIndex, c_pszBoneName, dwCRC);
}

#ifdef ENABLE_SKILL_COLOR_SYSTEM
uint32_t CActorInstance::AttachEffectByID(uint32_t dwParentPartIndex, const char* c_pszBoneName, uint32_t dwEffectID, const D3DXVECTOR3* c_pv3Position, uint32_t* dwSkillColor)
#else
	...
	for (uint32_t j = 0; j < CRaceData::PART_MAX_NUM; ++j)
	{
		...
		for (DWORD k = 0; k < pItemData->GetAttachingDataCount(); ++k)
		{
			...
			case NRaceData::ATTACHING_DATA_TYPE_EFFECT:
				...
				{
					StringPath(c_pAttachingData->pEffectData->strFileName);
					const uint32_t dwCRC = GetCaseCRC32(c_pAttachingData->pEffectData->strFileName.c_str(),
...
void CActorInstance::WikiRenderAllAttachingModuleEffect() const
{
