...
void CMapOutdoor::__RenderTerrain_RenderHardwareTransformPatch()
{
	...
	m_RenderedTextureFlagVector.assign(m_TextureSet.GetTextureCount(), 0);
...
void CMapOutdoor::__HardwareTransformPatch_RenderPatchSplat(int32_t patchnum, WORD wPrimitiveCount, D3DPRIMITIVETYPE ePrimitiveType)
{
	...
	for (uint32_t j = 1; j < pTerrain->GetNumTextures(); ++j)
	{
		...
		const size_t textureIndex = static_cast<size_t>(j);
		if (textureIndex < m_RenderedTextureFlagVector.size() && !m_RenderedTextureFlagVector[textureIndex])
		{
			m_RenderedTextureFlagVector[textureIndex] = 1;
...
void CMapOutdoor::__HardwareTransformPatch_RenderPatchNone(int32_t patchnum, WORD wPrimitiveCount, D3DPRIMITIVETYPE ePrimitiveType)
{
