...
CMapManager::CMapManager() : mc_pcurEnvironmentData(nullptr), m_pkMap(nullptr), m_isSoftwareTilingEnableReserved(false)
{
//	Initialize();
}
...
bool CMapManager::GetWaterHeight(int iX, int iY, int32_t* plWaterHeight)
{
	if (!m_pkMap)
	{
...
bool CMapManager::LoadEnvironmentData(const char * c_szFileName, TEnvironmentData * pEnvironmentData)
{
	...
	return Environment_Load(*pEnvironmentData, c_szFileName);
}
