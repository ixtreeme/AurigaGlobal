...
struct FGetShadowReceiverFromCollisionData
{
	...
	void operator ()(CGraphicObjectInstance* pInstance)
	{
		...
		if (TREE_OBJECT == pInstance->GetType() || ACTOR_OBJECT == pInstance->GetType() || EFFECT_OBJECT == pInstance->
			GetType())
			return;
...
struct FPCBlockerDistanceSort
{
	...
	bool operator () (CGraphicObjectInstance* plhs, CGraphicObjectInstance* prhs) const
	{
		...
		auto pos_l = plhs->GetPosition() - m_v3Eye;
		auto pos_r = prhs->GetPosition() - m_v3Eye;
		return D3DXVec3LengthSq(&pos_l) > D3DXVec3LengthSq(&pos_r);
	}
};
...
struct FGetShadowReceiverFromHeightData
{
	...
	CGraphicObjectInstance* m_apkShadowReceiver[COLLECT_MAX] = { 0 };
...
struct PCBlocker_SInstanceList
{
	...
	Item m_apkPCBlocker[CAPACITY] = { nullptr };
	...
	PCBlocker_SInstanceList(PCBlocker_CDynamicSphereInstanceVector* pkDSIVector) : m_dwInstCount(0), m_dwBlockerCount(0), m_dwBlockerOverCount(0), m_pkDSIVector(pkDSIVector)
	{
...
void CMapOutdoor::FPushTerrainToDeleteVector::operator () (CTerrain* pTerrain)
{
	...
	if (m_eLRDeleteDir == DELETE_LEFT && wCoordX < wReferenceCoordX - LOAD_SIZE_WIDTH) {
		m_ReturnTerrainVector.emplace_back(pTerrain);
	}
	else if (m_eLRDeleteDir == DELETE_RIGHT && wCoordX > wReferenceCoordX + LOAD_SIZE_WIDTH) {
		m_ReturnTerrainVector.emplace_back(pTerrain);
	}

	if (m_eTBDeleteDir == DELETE_TOP && wCoordY < wReferenceCoordY - LOAD_SIZE_WIDTH) {
		m_ReturnTerrainVector.emplace_back(pTerrain);
	}
	else if (m_eTBDeleteDir == DELETE_BOTTOM && wCoordY > wReferenceCoordY + LOAD_SIZE_WIDTH) {
		m_ReturnTerrainVector.emplace_back(pTerrain);
	}
}
...
{
	constexpr uint32_t dwTerrainEraseInterval = 1000 * 60;
...
void CMapOutdoor::UpdateAreaList(int32_t lCenterX, int32_t lCenterY)
{
	...
	FPushToDeleteVector::EDeleteDir eDeleteLRDir = (lCenterX > CTerrainImpl::XSIZE / 2) ? FPushToDeleteVector::DELETE_LEFT : FPushToDeleteVector::DELETE_RIGHT;

	FPushToDeleteVector::EDeleteDir eDeleteTBDir = (lCenterY > CTerrainImpl::YSIZE / 2) ? FPushToDeleteVector::DELETE_TOP : FPushToDeleteVector::DELETE_BOTTOM;
	...
	{
		m_TerrainDeleteVector = std::move(rPushTerrainToDeleteVector.m_ReturnTerrainVector);

		for (auto& terrain : m_TerrainDeleteVector)
		{
			auto it = std::find(m_TerrainVector.begin(), m_TerrainVector.end(), terrain);
			if (it != m_TerrainVector.end())
			{
				m_TerrainVector.erase(it);
			}
		}
	}
	...
	{
		m_AreaDeleteVector = std::move(rPushAreaToDeleteVector.m_ReturnAreaVector);

		for (auto& area : m_AreaDeleteVector)
		{
			auto it = std::find(m_AreaVector.begin(), m_AreaVector.end(), area);
			if (it != m_AreaVector.end())
			{
				m_AreaVector.erase(it);
			}
		}
	}
}
