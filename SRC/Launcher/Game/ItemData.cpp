...
void CItemData::__LoadFiles()
{
	...
	if (!m_strModelFileName.empty())
		m_pModelThing = static_cast<CGraphicThing*>(CResourceManager::Instance().GetResourcePointer(m_strModelFileName.c_str()));

	if (!m_strSubModelFileName.empty())
		m_pSubModelThing = static_cast<CGraphicThing*>(CResourceManager::Instance().GetResourcePointer(m_strSubModelFileName.c_str()));

	if (!m_strDropModelFileName.empty())
		m_pDropModelThing = static_cast<CGraphicThing*>(CResourceManager::Instance().GetResourcePointer(m_strDropModelFileName.c_str()));
	...
	if (!m_strLODModelFileNameVector.empty())
	{
		...
		for (uint32_t i = 0; i < m_strLODModelFileNameVector.size(); ++i)
		{
			...
			m_pLODModelThingVector[i] = static_cast<CGraphicThing*>(CResourceManager::Instance().GetResourcePointer(c_rstrLODModelFileName.c_str()));
		}
	}
}
...
void CItemData::__SetIconImage(const char * c_szFileName)
{
	if (!CResourceManager::Instance().IsFileExist(c_szFileName))
	{
		...
		if (CResourceManager::Instance().IsFileExist(c_szAlterIconImage))
			m_pIconImage = static_cast<CGraphicSubImage*>(CResourceManager::Instance().GetResourcePointer(c_szAlterIconImage));
#endif
	}
	else if (m_pIconImage == nullptr)
		m_pIconImage = static_cast<CGraphicSubImage*>(CResourceManager::Instance().GetResourcePointer(c_szFileName));
}
...
void CItemData::SetItemScale(const std::string strJob, const std::string strSex, const std::string strScaleX, const std::string strScaleY, const std::string strScaleZ, const std::string strPositionX, const std::string strPositionY, const std::string strPositionZ)
{
	...
	m_ScaleTable.tInfo[dwPos].fScaleX = std::stof(strScaleX) / 100.0f;
	m_ScaleTable.tInfo[dwPos].fScaleY = std::stof(strScaleY) / 100.0f;
	m_ScaleTable.tInfo[dwPos].fScaleZ = std::stof(strScaleZ) / 100.0f;
	m_ScaleTable.tInfo[dwPos].fPositionX = std::stof(strPositionX) * 100.0f;
	m_ScaleTable.tInfo[dwPos].fPositionY = std::stof(strPositionY) * 100.0f;
	m_ScaleTable.tInfo[dwPos].fPositionZ = std::stof(strPositionZ) * 100.0f;
}
