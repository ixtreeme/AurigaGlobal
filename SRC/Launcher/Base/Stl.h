...
template<typename TContainer>
void stl_wipe_second(TContainer& container)
{
...
template<typename T>
void safe_release(T& rpObject)
{
	if (!rpObject)
	{
	//	Tracenf("safe_release - object (rpObject) is null or empty pointer");
		return;
	}

	if constexpr (requires { rpObject.size(); })
	{
//		Tracenf("safe_release - object (rpObject) size: %zu", rpObject.size());
	}
	else
	{
		Tracenf("safe_release - releasing object (rpObject) at address: %p", rpObject);
	}
