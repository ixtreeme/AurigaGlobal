...
bool CTerrainImpl::LoadAttrMap(const char *c_szFileName)
{
	...
	const auto abFileData = (uint8_t *) lpcvFileData;
...
#pragma pack(pop)
		...
		if (constexpr uint16_t c_wAttrMapMagic = 2634; c_wAttrMapMagic != kAttrMapHeader.m_wMagic)
		{
		...
		const uint32_t dwFileRestSize=dwFileSize-sizeof(kAttrMapHeader);
		const uint32_t dwFileNeedSize=sizeof(m_abyAttrMap);
		...
		const uint8_t* abSrcAttrData= abFileData+sizeof(kAttrMapHeader);
...
#pragma pack(pop)
		...
		if (constexpr uint16_t c_wWaterMapMagic = 5426; c_wWaterMapMagic != kWaterMapHeader.m_wMagic)
		{
...
uint32_t CTerrainImpl::GetShadowMapColor(float fx, float fy) const
{
	float fMapSize = static_cast<float>(TERRAIN_XSIZE);
	float fooMapSize = 1.0f / fMapSize;
	if (fx < 0 || fy < 0 || fx >= fMapSize || fy >= fMapSize)
		return 0xFFFFFFFF;

	fx = fx * fooMapSize * static_cast<float>(SHADOWMAP_XSIZE - 1);
	fy = fy * fooMapSize * static_cast<float>(SHADOWMAP_YSIZE - 1);
	int ix, iy;
