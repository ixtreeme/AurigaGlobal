...
void CPythonSwitchbot::SetAttribute(uint8_t slot, uint8_t alternative, uint8_t attrIdx, uint8_t attrType, int attrValue)
{
...
void CPythonSwitchbot::AddAttributeToMap(const TSwitchbottAttributeTable& table)
{
	...
	{
		std::map<uint8_t, int32_t> attribute_map;
...
long CPythonSwitchbot::GetAttributeMaxValue(int iAttributeSet, uint8_t attrType)
{
	if (iAttributeSet == -1)
	{
		return 0;
...
PyObject* switchbotGetAttributeMaxValue(PyObject* poSelf, PyObject* poArgs)
{
	...
	int32_t maxValue = CPythonSwitchbot::Instance().GetAttributeMaxValue(iAttributeSet, bAttrType);
