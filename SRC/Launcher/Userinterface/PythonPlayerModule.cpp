...
{
	uint32_t iVID;
	if (!PyTuple_GetUnsignedLong(poArgs, 0, &iVID))
...
{
	CInstanceBase* pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	uint32_t iAlignmentPoint = 0;
	uint32_t iAlignmentGrade = 16;
	if (pInstance)
	{
...
PyObject* playerGetItemCountByVnumExtra(PyObject* poSelf, PyObject* poArgs)
{
	...
	uint32_t ItemNum = CPythonPlayer::Instance().GetItemCountbyVnumExtraInventory(ivnum);
	return Py_BuildValue("k", ItemNum);
}
#endif
...
PyObject* playerGetItemLink(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (pPlayerItem && CItemManager::Instance().GetItemDataPointer(pPlayerItem->vnum, &pItemData))
	{
		...
		len = _snprintf(itemlink, sizeof(itemlink), "item:%x:%x:%x:%x:%x:[{%d}]", pPlayerItem->vnum, pPlayerItem->flags, pPlayerItem->alSockets[0], pPlayerItem->alSockets[1], pPlayerItem->alSockets[2], pPlayerItem->vnum);
#else
		len = _snprintf(itemlink, sizeof(itemlink), "item:%x:%x:%x:%x:%x", pPlayerItem->vnum, pPlayerItem->flags, pPlayerItem->alSockets[0], pPlayerItem->alSockets[1], pPlayerItem->alSockets[2]);
#endif
		for (int i = 0; i < ITEM_ATTRIBUTE_SLOT_MAX_NUM; ++i)
		{
			...
			len += _snprintf(itemlink + len, sizeof(itemlink) - len, ":%x:%d", pPlayerItem->aAttr[i].bType, pPlayerItem->aAttr[i].sValue);
		}
...
PyObject* playerGetEmotionIconImage(PyObject* poSelf, PyObject* poArgs)
{
	...
	auto capsule = PyCapsule_New(m_kMap_iEmotionIndex_pkIconImage[iIndex], nullptr, nullptr);
	if (capsule)
	{
		return capsule;
	}
...
{
	int64_t dwLevel = CPythonPlayer::Instance().GetStatus(POINT_LEVEL);
...
void initPlayer()
{
	...
	PyModule_AddIntConstant(poModule, "EXTRA_INVENTORY6", POINT_EXTRA_INVENTORY6);
#endif
	...
	////
	PyModule_AddIntConstant(poModule, "PVM", POINT_ATTBONUS_MEDI_PVM);
	...
	PyModule_AddIntConstant(poModule, "HATSZOG_LADA_EVENT", HATSZOG_LADA_EVENT);
	PyModule_AddIntConstant(poModule, "BUPLA_RUN_BOSS_LOOT_EVENT", BUPLA_RUN_BOSS_LOOT_EVENT);
	PyModule_AddIntConstant(poModule, "MIKI_EVENT", MIKI_EVENT);
	PyModule_AddIntConstant(poModule, "KARI_EVENT", KARI_EVENT);
	PyModule_AddIntConstant(poModule, "DUPLA_SZILI_EVENT", DUPLA_SZILI_EVENT);
	PyModule_AddIntConstant(poModule, "COIN_EVENT", COIN_EVENT);
	PyModule_AddIntConstant(poModule, "DUPLA_BOSS_PONT_EVENT", DUPLA_BOSS_PONT_EVENT);
	PyModule_AddIntConstant(poModule, "DUPLA_RUN_PONT_EVENT", DUPLA_RUN_PONT_EVENT);

	PyModule_AddIntConstant(poModule, "VALENTIN_EVENT", VALENTIN_EVENT);

	PyModule_AddIntConstant(poModule, "GOLDEN_FROG_EVENT", GOLDEN_FROG_EVENT);
	PyModule_AddIntConstant(poModule, "TANAKA_EVENT", TANAKA_EVENT);
	PyModule_AddIntConstant(poModule, "EASTER_EVENT", EASTER_EVENT);
	PyModule_AddIntConstant(poModule, "LELEKGOMB_EVENT", LELEKGOMB_EVENT);
	...
	PyModule_AddIntConstant(poModule, "ATTRIBUTE_SLOT_EXTRA_RARE_NUM", ITEM_ATTRIBUTE_SLOT_EXTRA_RARE_NUM);
	PyModule_AddIntConstant(poModule, "ATTRIBUTE_SLOT_EXTRA_RARE_START", ITEM_ATTRIBUTE_SLOT_EXTRA_RARE_START);
	PyModule_AddIntConstant(poModule, "ATTRIBUTE_SLOT_EXTRA_RARE_END", ITEM_ATTRIBUTE_SLOT_EXTRA_RARE_END);
	...
	PyModule_AddIntConstant(poModule, "MOUNT_INVENTORY", MOUNT_INVENTORY);
	PyModule_AddIntConstant(poModule, "MOUNT_INVENTORY_SLOT_COUNT", c_Mount_Inventory_Count);
	PyModule_AddIntConstant(poModule, "SLOT_TYPE_MOUNT_INVENTORY", SLOT_TYPE_MOUNT_INVENTORY);
