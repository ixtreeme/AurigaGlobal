...
void CPythonSkill::__RegisterGradeIconImage(TSkillData& rData, const char* c_szHeader, const char* c_szImageName)
{
	for (int j = 0; j < SKILL_GRADE_COUNT; ++j)
	{
		...
		strFileName += szCount;
		strFileName += ".sub";
		rGradeData.pImage = static_cast<CGraphicImage*>(CResourceManager::Instance().GetResourcePointer(strFileName.c_str()));
	}
}
...
void CPythonSkill::__RegisterNormalIconImage(TSkillData& rData, const char* c_szHeader, const char* c_szImageName)
{
	...
	strFileName += c_szImageName;
	strFileName += ".sub";
	rData.pImage = static_cast<CGraphicImage*>(CResourceManager::Instance().GetResourcePointer(strFileName.c_str()));
	for (int j = 0; j < SKILL_GRADE_COUNT; ++j)
	{
		TGradeData& rGradeData = rData.GradeData[j];
...
bool CPythonSkill::RegisterSkill(uint32_t dwSkillIndex, const char* c_szFileName)
{
...

	{
		...
		if (TextFileLoader.GetTokenVector(szGradeData, &pGradeDataVector))
		{
			...
			for (uint32_t i = 0; i < min(SKILL_GRADE_COUNT, pGradeDataVector->size() / 2); ++i)
			{
...
float CPythonSkill::SSkillData::ProcessFormula(CPoly* pPoly, float fSkillLevel, int iMinMaxType)
{
	if (pPoly->Analyze())
	{
		for (uint32_t i = 0; std::cmp_less(i, pPoly->GetVarCount()); ++i)
		{
			...
			else
			{
				...
				fState = static_cast<float>(iState);
...
PyObject* skillGetIconInstance(PyObject* poSelf, PyObject* poArgs)
{
	...
	auto capsule = PyCapsule_New(pImageInstance, nullptr, nullptr);
	if (capsule)
	{
		return capsule;
	}
...
PyObject* skillGetIconImageNew(PyObject* poSelf, PyObject* poArgs)
{
	...
	auto capsule = PyCapsule_New(c_pSkillData->GradeData[iGradeIndex].pImage, nullptr, nullptr);
	if (capsule)
	{
		return capsule;
	}
...
{
	PyObject* iHandle;
	if (!PyTuple_GetObject(poArgs, 0, &iHandle))
		return Py_BadArgument();

	if (!iHandle)
		Py_BadArgument();

	auto CGraphicImageInstancefasz = static_cast<CGraphicImageInstance*>(PyCapsule_GetPointer(iHandle, nullptr));

	CGraphicImageInstance::Delete(CGraphicImageInstancefasz);
