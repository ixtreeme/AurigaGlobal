...
static std::vector<uint32_t> s_itemVidList;
static uint32_t s_dwNextPickClear = 0;
...
void CPythonPlayer::PickCloseItem()
{
	...
	if (dwCurTime >= s_dwNextPickClear) {
		s_itemVidList.clear();
		s_dwNextPickClear = dwCurTime + 100;
	}
	...
	int size = itemVidList.size();
	if (size == 0) {
		return;
	}

	for (int i = 0; i < size; i++) {
		uint32_t dwItemID = itemVidList[i];
		if (dwItemID != 0) {
			auto iter = std::find(s_itemVidList.begin(), s_itemVidList.end(), dwItemID);
			if (iter == s_itemVidList.end()) {
				SendClickItemPacket(dwItemID);
				s_itemVidList.push_back(dwItemID);
			}
		}
	}
}
...
void CPythonPlayer::PickCloseItem()
{
	...
	uint32_t dwItemID;
	CPythonItem& rkItem=CPythonItem::Instance();
	if (!rkItem.GetCloseItem(kPPosMain, &dwItemID, 600/*__GetPickableDistance()*/))
		return;
