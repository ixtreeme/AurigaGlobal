...
private:
	std::unordered_map<uint32_t, std::string> m_mapPendingMountCounts;
#endif
	public:
#ifdef ENABLE_MOUNT_COUNT_ABOVE_CHAR_RAZOR93
		void StorePendingMountCount(uint32_t dwVID, const std::string& szText);
		void ApplyPendingMountCountIfExists(CInstanceBase* pInstance);
#endif
		// Character List
