...
#include "../Python/PythonWindowManager.h"
...
void CPythonMiniMap::RegisterAtlasMarkBoss(uint8_t byType, const char * c_szName, int32_t lx, int32_t ly, int32_t lTime)
{
...
void CPythonMiniMap::__LoadAtlasMarkInfo()
{
	...
	for (uint32_t i = 0; i < stTokenVectorMap.size(); ++i)
	{
		...
		if (rVector.size() == 3)
		{
			...
			aAtlasMarkInfo.m_fX = std::stof(c_rstrPositionX);
			aAtlasMarkInfo.m_fY = std::stof(c_rstrPositionY);
...
#ifdef ENABLE_ATLAS_BOSS
bool CPythonMiniMap::GetAtlasInfo(float fScreenX, float fScreenY, std::string & rReturnString, float * pReturnPosX, float * pReturnPosY, uint32_t * pdwTextColor, uint32_t * pdwGuildID, long * lTime)
#else
