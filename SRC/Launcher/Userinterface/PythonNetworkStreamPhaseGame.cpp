...
void CPythonNetworkStream::GamePhase()
{
	...
	const uint32_t MAX_RECV_COUNT = 8;
...
case HEADER_GC_ITEM_ON_TITLE_NAME_UPDATE:
{
	TPacketGCItemOnTitleNameUpdate kPacket;
	if (!Recv(sizeof(kPacket), &kPacket))
	{
		TraceError("RecvItemOnTitleNameUpdate: packet read failed");
		ret = false;
		break;
	}

	kPacket.name[sizeof(kPacket.name) - 1] = '\0';


	// Visual-only: do NOT change character's real name, only update TextTail prefix
	CPythonTextTail::Instance().SetItemOnTitlePrefix(kPacket.dwVID, kPacket.name);
	CPythonTextTail::Instance().UpdateItemOnTitleTextTail(kPacket.dwVID);

#ifdef _DEBUG
	TraceError("ITEM_ON_TITLE: VID=%u PREFIX=%s", kPacket.dwVID, kPacket.name);
#endif
	break;
}
#endif
#ifdef ENABLE_MOUNT_COUNT_ABOVE_CHAR_RAZOR93
		case HEADER_GC_MOUNT_COUNT_OVERHEAD:
		{
			...
			TraceError("RecvMountCountOverhead: Beolvasott VID=%u, mountCount=%d\n", kPacket.dwVID, kPacket.mountCount);

			CPythonCharacterManager& rkChrMgr = CPythonCharacterManager::Instance();
			CInstanceBase* pInst = rkChrMgr.GetInstancePtr(kPacket.dwVID);


			char szMountCount[32];
			snprintf(szMountCount, sizeof(szMountCount), "Mount: %d", kPacket.mountCount);
			TraceError("Mount szöveg előállítva: %s\n", szMountCount);


			if (!pInst)
			{
				TraceError("RecvMountCountOverhead: ❌ NEM található karakter ezzel a VID-del: %u\n", kPacket.dwVID);

				rkChrMgr.StorePendingMountCount(kPacket.dwVID, szMountCount);

			}
			else
			{
				TraceError("RecvMountCountOverhead: ✅ Talált karakter, SetMountCountOverhead hívás indul\n");

				pInst->SetMountCountOverhead(szMountCount);
		...
		case HEADER_GC_MOUNT_INVENTORY:
			ret = RecvMountInventoryPacket();
			break;
		...
		case HEADER_GC_FAKE_SHOP_SIGN:
			ret = CPythonNetworkStream::Instance().RecvFakeShopSignPacket();
			break;
#endif
#ifdef LEADERBOARD_RAZOR93
		case HEADER_GC_LEADERBOARD_DATA:
		{
			TPacketGCLeaderboard packet;
		...
		case HEADER_GC_LEADERBOARD_NEWS:
		{
			TPacketGCLeaderboardNews packet;
			if (!Recv(sizeof(packet), &packet))
				return;
			...
			//TraceError("[LB] SKILLMOB packet received: %.100s", packet.data);
			
			CPythonLeaderboard::Instance().SetTextSkillMob(packet.data);
			CPythonLeaderboard::Instance().DrawSkillMob();
			ReplaceTextureGlobalByFilenameLoose("image0.tga", CPythonLeaderboard::Instance().GetTexSkillMob());
			//CPythonLeaderboard::Instance().OnLeaderBoardRender();
			break;
		}
		case HEADER_GC_LEADERBOARD_GUILD:
		{
			TPacketGCLeaderboard packet;
			if (!Recv(sizeof(packet), &packet))
				return;
			...
			CPythonLeaderboard::Instance().SetTextGuild(packet.data);
			CPythonLeaderboard::Instance().DrawGuild();

			// FONTOS: NEM image0.tga, hanem clan.tga!
			ReplaceTextureGlobalByFilenameLoose("clan.tga", CPythonLeaderboard::Instance().GetTexGuild());
			break;
		}
#endif
...
void CPythonNetworkStream::Warp(int32_t lGlobalX, int32_t lGlobalY)
{
	...
	if (CPythonLeaderboard::HasInstance())
	{
		CPythonLeaderboard::Instance().ResetBindings();
	}
#endif
...
bool CPythonNetworkStream::RecvWarpPacket()
{
	...
	CNetworkStream::Connect(kWarpPacket.lAddr, kWarpPacket.wPort);
...
bool CPythonNetworkStream::SendCharacterStatePacket(const TPixelPosition& c_rkPPosDst, float fDstRot, uint8_t eFunc, uint8_t uArg)
{
	...
	kStatePacket.lX = static_cast<int32_t>(c_rkPPosDst.x);
	kStatePacket.lY = static_cast<int32_t>(c_rkPPosDst.y);
...
bool CPythonNetworkStream::SendChatPacket(const char* c_szChat, uint8_t byType)
{
	...
	ChatPacket.length = (uint16_t)sizeof(ChatPacket) + iTextLen;
...
bool CPythonNetworkStream::RecvChatPacket()
{
	...
	int uChatSize = kChat.size - sizeof(kChat);
	...
	if (kChat.dwVID != 0)
	{
		...
		if (CHAT_TYPE_SHOUT != kChat.type)
		{
			...
			while (firstDelPos > 0 && secondDelPos > 0) {
				...
				uint32_t vnum = stoi(chatStr.substr(firstDelPos + 2, secondDelPos - firstDelPos - 2));
...
bool CPythonNetworkStream::RecvWhisperPacket()
{
		...
		)
	{
		...
		while (firstDelPos > 0 && secondDelPos > 0) {
			replace = chatStr.substr(firstDelPos + 1, secondDelPos - firstDelPos);
			const int vnum = stoi(chatStr.substr(firstDelPos + 2, secondDelPos - firstDelPos - 2));
			CItemManager::Instance().SelectItemData(vnum);
			CItemData* pItemData = CItemManager::Instance().GetSelectedItemDataPointer();
		...
		if (POINT_GOLD == PointChange.Type)
		{
			if (PointChange.amount > 0)
			{
				PyCallClassMemberFunc(
					...
					"OnPickMoney",
					Py_BuildValue("(L)", (long long)PointChange.amount)
				);
...
bool CPythonNetworkStream::RecvMotionPacket()
{
	...
	if (0 != MotionPacket.victim_vid)
		CPythonCharacterManager::Instance().GetInstancePtr(MotionPacket.victim_vid);
...
bool CPythonNetworkStream::RecvShopPacket()
{
	...
	case SHOP_SUBHEADER_GC_START_EX:
	{
		...
		{
			auto pPackTab = (TPacketGCShopStartEx::TSubPacketShopTab*)&vecBuffer[read_point];
			read_point += sizeof(TPacketGCShopStartEx::TSubPacketShopTab);

			CPythonShop::instance().SetTabCoinType((uint8_t)i, pPackTab->coin_type);
			CPythonShop::instance().SetTabName((uint8_t)i, pPackTab->name);
			...
			for (uint8_t j = 0; j < SHOP_HOST_ITEM_MAX_NUM; j++)
			{
				...
				CPythonShop::Instance().SetItemData((uint8_t)i, j, *itemData);
			}
		}
	...
	case SHOP_SUBHEADER_GC_UPDATE_PRICE:
	{
		...
		PyTuple_SetItem(args, 0, PyLong_FromLongLong(*(int64_t*)&vecBuffer[0]));
...
bool CPythonNetworkStream::RecvExchangePacket()
{
	...
	case EXCHANGE_SUBHEADER_GC_ITEM_ADD:
		if (exchange_packet.is_me)
		{
			...
			PyTuple_SetItem(args, 0, PyLong_FromLongLong(exchange_packet.arg4.window_type));
			PyTuple_SetItem(args, 1, PyLong_FromLongLong(exchange_packet.arg4.cell));
			PyTuple_SetItem(args, 2, PyLong_FromLongLong(iSlotIndex));
...
bool CPythonNetworkStream::WheelDestiny(const uint8_t option)
{
	if (!__CanActMainInstance())
	{
		return true;
...
bool CPythonNetworkStream::SendSyncPositionElementPacket(uint32_t dwVictimVID, int32_t dwVictimX, int32_t dwVictimY)
{
...
bool CPythonNetworkStream::RecvGuild()
{
	...
	case GUILD_SUBHEADER_GC_GUILD_NAME:
	{
		...
		uint8_t byGuildLevel;
#endif

		int iPacketSize = int(GuildPacket.size) - sizeof(GuildPacket);

#ifdef ENABLE_GUILD_RENEWAL_BY_RAZOR93
		int nItemSize = sizeof(dwID) + sizeof(byGuildLevel) + GUILD_NAME_MAX_LEN;
#else
...
#ifdef ENABLE_GUILD_RENEWAL_BY_RAZOR93
			if (!Recv(sizeof(byGuildLevel), &byGuildLevel))
				return false;
#endif
...
#ifdef ENABLE_GUILD_RENEWAL_BY_RAZOR93
			CPythonGuild::Instance().RegisterGuildName(dwID, szGuildName, byGuildLevel);
#else
...
//		Py_BuildValue("(is)", p.dwVID, mountText.c_str()));
//
//	return true;
//}
...
bool CPythonNetworkStream::RecvFakeShopSignPacket()
{
	...
	CInstanceBase* pInst = CPythonCharacterManager::Instance().GetInstancePtr(p.dwVID);
	if (!pInst || !pInst->IsPC())
	{
		TraceError(">>> RecvFakeShopSignPacket: Skipped non-PC instance. VID = %u\n", p.dwVID);
		return true;
	}

	PyObject* poGameWnd = m_apoPhaseWnd[PHASE_WINDOW_GAME];
	if (!poGameWnd)
	{
		// Loading  
		TraceError(">>> RecvFakeShopSignPacket: GameWnd is null (phase not ready). VID=%u\n", p.dwVID);
		return true;
	}

	PyCallClassMemberFunc(poGameWnd,
		"BINARY_MountSign_AppearRazor93",
		Py_BuildValue("(iii)", (int)p.dwVID, p.iMountCount, p.iBeltCount));

	Tracef(">>> RecvFakeShopSignPacket OK: VID=%u, Mount=%d, Belt=%d\n",
		p.dwVID, p.iMountCount, p.iBeltCount);
...
bool CPythonNetworkStream::SendClientVersionPacket()
{
	...
	if (LocaleService_IsEUROPE() && false == LocaleService_IsYMIR())
	{
		...
		strncpy(kVersionPacket.timestamp, "20251204", sizeof(kVersionPacket.timestamp) - 1); 
...
bool CPythonNetworkStream::RecvAffectAddPacket()
{
	...
	if (rkElement.dwType == CInstanceBase::NEW_AFFECT_AUTO_METIN_FARM)
		CPythonPlayer::Instance().SetTotalAutoFarmCount(AUTO_QUQUE_ATTACK_MAX_TARGET);
#endif
...
#ifdef ENABLE_BATTLE_PASS
bool CPythonNetworkStream::SendBattlePassAction(uint8_t bAction)
{
	if (!__CanActMainInstance())
		return true;
...
#ifdef ENABLE_MULTI_LANGUAGE
bool CPythonNetworkStream::SendChangeLanguagePacket(uint8_t bLanguage)
{
...
void CPythonNetworkStream::Discord_Update(int lvl)
{
	...
	discordPresence.state = "www.Auriga-Global.eu";
	discordPresence.details = "Global Server";
	discordPresence.largeImageKey = "logo";
	discordPresence.largeImageText = "Auriga-Global - PvM Medium";
...
bool CPythonNetworkStream::SendWikiRequestInfo(unsigned long long retID, uint32_t vnum, bool isMob)
{
...
bool CPythonNetworkStream::RecvWikiPacket()
{
	...
	else
	{
		...
		CPythonNonPlayer::TWikiInfoTable* mobData = CPythonNonPlayer::instance().GetWikiTable(mob_data.vnum);
		if (!mobData)
		{
...
bool CPythonNetworkStream::RecvItemShop()
{
	...
	{
		uint32_t dragonCoin;
		if (!Recv(sizeof(uint32_t), &dragonCoin))
			return false;
		bool isLogOpen;
		...
		if (isLogOpen)
		{
			...
			if (!Recv(sizeof(TIShopLogData), &logData))
				return false;
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "ItemShopAppendLog", Py_BuildValue("(sissiiI)", logData.buyDate, logData.buyTime, logData.playerName, logData.ipAdress, logData.itemVnum, logData.itemCount, logData.itemPrice));
		}

		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "ItemShopSetDragonCoin", Py_BuildValue("(I)", dragonCoin));
	}
	break;
	case ITEMSHOP_LOG:
	{
		...
		if (logCount)
		{
			...
			if (!Recv(sizeof(TIShopLogData) * logCount, &m_vec[0]))
				return false;
			for (uint32_t j = 0; j < m_vec.size(); ++j)
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "ItemShopAppendLog", Py_BuildValue("(sissiiI)", m_vec[j].buyDate, m_vec[j].buyTime, m_vec[j].playerName, m_vec[j].ipAdress, m_vec[j].itemVnum, m_vec[j].itemCount, m_vec[j].itemPrice));
		}
	...
	case ITEMSHOP_LOAD:
	{
		...
		{
			uint8_t categoryIndex, categorySize;
			if (!Recv(sizeof(uint8_t), &categoryIndex))
				return false;
			if (!Recv(sizeof(uint8_t), &categorySize))
				return false;
			for (DWORD x = 0; x < categorySize; ++x)
			{
				uint8_t categorySubIndex, categorySubSize;
				if (!Recv(sizeof(uint8_t), &categorySubIndex))
					return false;
				if (!Recv(sizeof(uint8_t), &categorySubSize))
					return false;
				...
				if (categorySubSize)
				{
					...
					for (DWORD k = 0; k < m_vec.size(); ++k)
					{
						...
						PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "ItemShopAppendItem", Py_BuildValue("(iiiiIiiiiI)",
