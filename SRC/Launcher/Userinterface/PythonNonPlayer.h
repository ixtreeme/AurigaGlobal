...
#pragma pack(1)
		...
		{
			static bool IsValidStruct(uint64_t structSize)
			{
				switch (structSize)
				{
...
#pragma pack(pop)
	...
	public:
		...
		size_t								WikiLoadClassMobs(uint8_t bType, unsigned short fromLvl, unsigned short toLvl);
		...
		uint32_t				GetMonsterType(uint32_t dwVnum);
		uint32_t				GetMonsterRank(uint32_t dwVnum);
