...
#include "stdafx.h"
#include "../Render/CullingManager.h"
#include "../Render/Camera.h"
#include "../Pack/EterPackManager.h"
#include "../Game/MapOutDoor.h"
#include "../Game/PropertyLoader.h"
...
{
	EnableTerrainOnlyForHeight(true);
}

void CPythonBackground::EnableTerrainOnlyForHeight(bool bFlag)
{
	if (!m_pkMap)
		return;
	
	CMapOutdoor& rkMap=GetMapOutdoorRef();
	rkMap.EnableTerrainOnlyForHeight(bFlag);
}
...
{
	rGlobalX-=(int32_t)m_dwBaseX;
	rGlobalY-= (int32_t)m_dwBaseY;
}

void CPythonBackground::LocalPositionToGlobalPosition(int32_t& rLocalX, int32_t& rLocalY)
{
	rLocalX+= (int32_t)m_dwBaseX;
	rLocalY+= (int32_t)m_dwBaseY;
}
...
void CPythonBackground::Warp(uint32_t dwX, uint32_t dwY)
{
	...
	{
		rkMap.EnableTerrainOnlyForHeight(false);
		rkMap.EnablePortal(false);
	}
