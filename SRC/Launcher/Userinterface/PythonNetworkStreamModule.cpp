...
PyObject* netSendSafeboxCheckinPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	case 2:
		...
		if (!PyTuple_GetInteger(poArgs, 0, &InventoryPos.cell))
			return Py_BuildException();
		if (!PyTuple_GetUnsignedLong(poArgs, 1, &iSafeBoxPos))
			return Py_BuildException();
		break;
	case 3:
		...
		if (!PyTuple_GetInteger(poArgs, 1, &InventoryPos.cell))
			return Py_BuildException();
		if (!PyTuple_GetUnsignedLong(poArgs, 2, &iSafeBoxPos))
			return Py_BuildException();
		break;
...
PyObject* netSendSafeboxCheckoutPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	{
	case 2:
		if (!PyTuple_GetUnsignedLong(poArgs, 0, &iSafeBoxPos))
			return Py_BuildException();
		if (!PyTuple_GetInteger(poArgs, 1, &InventoryPos.cell))
			return Py_BuildException();
		break;
...
{
	uint32_t iSourcePos;
	if (!PyTuple_GetUnsignedLong(poArgs, 0, &iSourcePos))
		return Py_BuildException();
	int iTargetPos;
...
PyObject* netSendMountInventoryCheckinPacket(PyObject* poSelf, PyObject* poArgs)
{
	TItemPos inventoryPos;
	uint32_t mountPos = 0;
	...
	case 2:
		...
		if (!PyTuple_GetInteger(poArgs, 0, &inventoryPos.cell))
			return Py_BuildException();
		if (!PyTuple_GetUnsignedLong(poArgs, 1, &mountPos))
			return Py_BuildException();
		break;
	case 3:
		if (!PyTuple_GetInteger(poArgs, 0, &inventoryPos.window_type))
		...
		if (!PyTuple_GetInteger(poArgs, 1, &inventoryPos.cell))
			return Py_BuildException();
		if (!PyTuple_GetUnsignedLong(poArgs, 2, &mountPos))
			return Py_BuildException();
		break;
	default:
		return Py_BuildException();
	...
	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	rns.SendMountInventoryCheckinPacket(inventoryPos, static_cast<uint16_t>(mountPos));

	return Py_BuildNone();
}

PyObject* netSendMountInventoryCheckoutPacket(PyObject* poSelf, PyObject* poArgs)
{
	uint32_t mountPos = 0;
	TItemPos inventoryPos;
	...
	{
	case 2:
		if (!PyTuple_GetUnsignedLong(poArgs, 0, &mountPos))
			return Py_BuildException();
		if (!PyTuple_GetInteger(poArgs, 1, &inventoryPos.cell))
			return Py_BuildException();
		inventoryPos.window_type = INVENTORY;
	...
	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	rns.SendMountInventoryCheckoutPacket(static_cast<uint16_t>(mountPos), inventoryPos);

	return Py_BuildNone();
}

PyObject* netSendMountInventoryItemMovePacket(PyObject* poSelf, PyObject* poArgs)
{
	uint32_t sourcePos;
	uint32_t destPos;

	if (!PyTuple_GetUnsignedLong(poArgs, 0, &sourcePos))
		return Py_BuildException();
	if (!PyTuple_GetUnsignedLong(poArgs, 1, &destPos))
		return Py_BuildException();

	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	rns.SendMountInventoryItemMovePacket(static_cast<uint16_t>(sourcePos), static_cast<uint16_t>(destPos));
...
PyObject* netSendCharacterPinCodePacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendCharacterPinCodePacket((uint8_t)iIndex, szPinCode);
	return Py_BuildNone();
}
#endif
...
void initnet()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "SendMountInventoryCheckinPacket",		netSendMountInventoryCheckinPacket,		METH_VARARGS },
		{ "SendMountInventoryCheckoutPacket",	netSendMountInventoryCheckoutPacket,	METH_VARARGS },
		{ "SendMountInventoryItemMovePacket",	netSendMountInventoryItemMovePacket,	METH_VARARGS },
	...
	PyModule_AddIntConstant(poModule, AY_OBFUSCATE("PHASE_WINDOW_LOGIN"), CPythonNetworkStream::PHASE_WINDOW_LOGIN);
