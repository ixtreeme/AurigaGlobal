...
bool CPythonItem::TGroundItemInstance::Update()
{
	...
	if (dwEndTime < CTimer::Instance().GetCurrentMillisecond())
	{
		...
		XMConvertToRadians(rEnd.y),
		XMConvertToRadians(rEnd.x),
		XMConvertToRadians(rEnd.z));
	...
	else
	{
		...
		D3DXMATRIX mat;
		D3DXMatrixRotationYawPitchRoll(&mat,
		XMConvertToRadians(rEnd.y + etime*rStart.y),
		XMConvertToRadians(rEnd.x + etime*rStart.x),
		XMConvertToRadians(rEnd.z + etime*rStart.z));
...
{
	for (const auto& itor : m_GroundItemInstanceMap)
	{
...
std::vector<uint32_t> CPythonItem::GetCloseItemVector(const std::string& myName, const TPixelPosition& c_rPixelPosition) const
{
...
void CPythonItem::CreateItem(uint32_t dwVirtualID, uint32_t dwVirtualNumber, float x, float y, float z, bool bDrop)
{
	...
	if (bDrop)
	{
		...
		float rot = frandom(0.0f, 2.0f * std::numbers::pi_v<float>);
...
CPythonItem::CPythonItem() : m_dwDropItemEffectID(0), m_nMouseX(0), m_nMouseY(0)
{
