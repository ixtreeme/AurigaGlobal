...
//#include "AbyssAntiCheat.h"
//#include "AbyssIntegrity.h"
#include "Packet.h"
...
typedef uint64_t(_stdcall* GetVerificationCodeFunc)();
typedef bool(_stdcall* VerifyClientResponseFunc)(uint64_t);
...
bool CAccountConnector::__AuthState_RecvPhase()
{
	...
	else if (kPacketPhase.phase == PHASE_AUTH)
	{
		...
		{
			InitIxacIntegrityWatchdog();
		}*/

		/*HMODULE hAntiCheat = LoadLibraryA(AY_OBFUSCATE("IXAC.dll"));
		if (!hAntiCheat) {


			LoadLibrary(AY_OBFUSCATE("IXAC.dll"));
		}*/
		...
		GetVerificationCodeFunc getCode = (GetVerificationCodeFunc)GetProcAddress(hAntiCheat, AY_OBFUSCATE("GetVerificationCode"));
		if (!getCode) {

			FreeLibrary(hAntiCheat);
			ExitProcess(1);
			return false;
		}

		VerifyClientResponseFunc verifyResponse = (VerifyClientResponseFunc)GetProcAddress(hAntiCheat, AY_OBFUSCATE("VerifyClientResponse"));
		if (!verifyResponse) {
		...
		uint64_t verificationCode = getCode();

		uint64_t response = verificationCode + 1;
