...
void CPythonMessenger::AppendLanguage(const char * name, const char * lang)
{
	PyCallClassMemberFunc(m_poMessengerHandler, "AppendLanguage", Py_BuildValue("(ss)", name, lang));
}
#endif
...
void CPythonMessenger::OnHelperLogin(const char * c_szKey)
{
	m_HelperNameMap.insert(c_szKey);

	PyCallClassMemberFunc(m_poMessengerHandler, "OnLogin", Py_BuildValue("(is)", MESSENGER_GROUP_INDEX_HELPER, c_szKey));
}

void CPythonMessenger::OnHelperLogout(const char * c_szKey)
{
	m_HelperNameMap.insert(c_szKey);

	if (m_poMessengerHandler)
	{
		PyCallClassMemberFunc(m_poMessengerHandler, "OnLogout", Py_BuildValue("(is)", MESSENGER_GROUP_INDEX_HELPER, c_szKey));
	}
}
#endif
