...
PyObject* appGetImageInfo(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				hr = pFrame->GetSize(&uWidth, &uHeight);
				if (SUCCEEDED(hr))
					canLoad = true;
			}
		}
	...
	return Py_BuildValue("iii", canLoad ? 1 : 0, uWidth, uHeight);
}
...
//		return Py_BuildException();
//
//	bool ret = CPythonLauncherIxtreeme::Instance().RunFile(szFileName);
...
namespace
{
	constexpr const char* kTextFileLoaderCapsuleName = "TextFileLoader";

	void TextFileLoaderCapsuleDestructor(PyObject* capsule)
	{
		auto* loader = static_cast<CTextLineLoader*>(PyCapsule_GetPointer(capsule, kTextFileLoaderCapsuleName));
		if (!loader)
		{
			PyErr_Clear();
			return;
...
PyObject * appOpenTextFile(PyObject * poSelf, PyObject * poArgs)
{
	...
	return PyCapsule_New(pTextLineLoader, kTextFileLoaderCapsuleName, TextFileLoaderCapsuleDestructor);
}

PyObject* appCloseTextFile(PyObject* poSelf, PyObject* poArgs)
{
	PyObject* pyHandle;
	if (!PyArg_ParseTuple(poArgs, "O", &pyHandle))
		return Py_BuildException();

	if (!PyCapsule_CheckExact(pyHandle)) {
		PyErr_SetString(PyExc_TypeError, "Expected a PyCapsule object");
		return nullptr;
	}

	CTextLineLoader* pTextFileLoader = static_cast<CTextLineLoader*>(PyCapsule_GetPointer(pyHandle, kTextFileLoaderCapsuleName));
	if (!pTextFileLoader) {
...
PyObject* appGetTextFileLine(PyObject* poSelf, PyObject* poArgs)
{
	PyObject* pyHandle;
	int iLineIndex;
	if (!PyArg_ParseTuple(poArgs, "Oi", &pyHandle, &iLineIndex))
		return Py_BuildException();
	...
	/*int iLineIndex;
	if (!PyArg_ParseTuple(poArgs, "i", &iLineIndex))
		return Py_BuildException();*/
...
#else
	ShellExecuteA(nullptr, "open", szName, NULL, nullptr, SW_SHOWNORMAL);
#endif
#else
#ifdef UNICODE
...
void initapp()
{
	...
	PyModule_AddIntConstant(poModule, "ENABLE_GUILD_ATTRIBUTE", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_GUILD_ATTRIBUTE", 0);
#endif
...
PyModule_AddIntConstant(poModule, "ENABLE_CLIENT_OPTIMIZATION", 0);
	...
	PyModule_AddIntConstant(poModule, "ENABLE_CHRISTMAS_WHEEL_OF_DESTINY", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_CHRISTMAS_WHEEL_OF_DESTINY", false);
#endif
