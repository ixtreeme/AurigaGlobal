...
PyObject* eventGetMapCount(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonEventManager::Instance().GetMapCount());
}


PyObject* eventGetMapPrice(PyObject* poSelf, PyObject* poArgs)
{
	int iMapIndex = 0;
	if (!PyTuple_GetInteger(poArgs, 0, &iMapIndex))
		return Py_BuildException();

	return Py_BuildValue("i", CPythonEventManager::instance().GetMapPrice(iMapIndex));
}


PyObject* eventGetMapTitle(PyObject* poSelf, PyObject* poArgs)
{
	int iMapIndex = 0;
	if (!PyTuple_GetInteger(poArgs, 0, &iMapIndex))
		return Py_BuildException();

	return Py_BuildValue("s", CPythonEventManager::instance().GetMapTitle(iMapIndex));
}


PyObject* eventGetMapDesc(PyObject* poSelf, PyObject* poArgs)
{
	int iMapIndex = 0;
	if (!PyTuple_GetInteger(poArgs, 0, &iMapIndex))
		return Py_BuildException();

	return Py_BuildValue("s", CPythonEventManager::instance().GetMapDesc(iMapIndex));
}


PyObject* eventGetMapItems(PyObject* poSelf, PyObject* poArgs)
{
	int iMapIndex = 0;
	if (!PyTuple_GetInteger(poArgs, 0, &iMapIndex))
		return Py_BuildException();

	CPythonEventManager::MAPITEMS* pItems = nullptr;
	CPythonEventManager::instance().GetMapItems(iMapIndex, &pItems);

	if (!pItems)
		return Py_BuildException();

	PyObject* pyTuple = PyTuple_New(pItems->size());

	for (auto i = 0; i < pItems->size(); i++)
		PyTuple_SetItem(pyTuple, i, Py_BuildValue("i", pItems->at(i)));

	return pyTuple;
}


PyObject* eventGetMapButtonPosition(PyObject* poSelf, PyObject* poArgs)
{
	int iMapIndex = 0;
	if (!PyTuple_GetInteger(poArgs, 0, &iMapIndex))
		return Py_BuildException();
	...
	int x = 0, y = 0;
	CPythonEventManager::instance().GetMapButtonPosition(iMapIndex, &x, &y);
	return Py_BuildValue("(ii)", x, y);
}



PyObject* eventGetMapLevel(PyObject* poSelf, PyObject* poArgs)
{
	int iMapIndex = 0;
	if (!PyTuple_GetInteger(poArgs, 0, &iMapIndex))
		return Py_BuildException();

	int iLevel = CPythonEventManager::instance().GetMapLevel(iMapIndex);
	return Py_BuildValue("i", iLevel);
}

PyObject* eventGetMapLevelMax(PyObject* poSelf, PyObject* poArgs)
{
	int iMapIndex = 0;
	if (!PyTuple_GetInteger(poArgs, 0, &iMapIndex))
		return Py_BuildException();

	int iLevelMax = CPythonEventManager::instance().GetMapLevelMax(iMapIndex);
	return Py_BuildValue("i", iLevelMax);
}
#endif
...
void initEvent()
{
	static PyMethodDef s_methods[] =
	{
		...
		#ifdef ENABLE_MAP_TELEPORTER
		{ "GetMapCount",				eventGetMapCount,					METH_VARARGS },
		{ "GetMapPrice",				eventGetMapPrice,					METH_VARARGS },
		{ "GetMapTitle",				eventGetMapTitle,					METH_VARARGS },
		{ "GetMapDesc",					eventGetMapDesc,					METH_VARARGS },
		{ "GetMapItems",				eventGetMapItems,					METH_VARARGS },
		{ "GetMapButtonPosition",		eventGetMapButtonPosition,			METH_VARARGS },
		{ "GetMapLevel",				eventGetMapLevel,					METH_VARARGS },
		{ "GetMapLevelMax",				eventGetMapLevelMax,					METH_VARARGS },
#endif
