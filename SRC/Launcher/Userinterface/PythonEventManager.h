...
public:
	typedef struct SEventSet
	{
		...
		SEventSet() : ix(0), iy(0), iWidth(0), iyLocal(0), isLock(false), lLastDelayTime(0), iCurrentLetter(0),
			CurrentColor(D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f)), strCurrentLine(""), pCurrentTextLine(nullptr),
			isConfirmWait(false), pConfirmTimeTextLine(nullptr), iConfirmEndTime(0),
			iVisibleStartLine(0), iVisibleLineCount(0), iAdjustLine(0),
			DiffuseColor(D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f)), lWaitingTime(0), iRestrictedCharacterCount(0),
			nAnswer(0), isTextCenterMode(false), isWaitFlag(false), poEventHandler(nullptr)
		{
	...
	enum eMapConst
	{
		MAP_TITLE_MAX_LEN = 24,
		MAP_DESC_MAX_LEN = 1024,
	};
	...
	struct TMapConfig
	{
		char szTitle[MAP_TITLE_MAX_LEN];
		char szDesc[MAP_DESC_MAX_LEN];
		...
		TMapConfig() : lButtonX(0), lButtonY(0), iLevel(0), iLevelMax(0), price(0), items()
		{
			szTitle[0] = '\0';
			szDesc[0] = '\0';
		}
...
public:
	...
	void			LoadMapConfig();
	int				GetMapCount();
	uint32_t			GetMapPrice(int index);
	const char* GetMapTitle(int index);
	const char* GetMapDesc(int index);
	void			GetMapItems(int index, MAPITEMS** ppData);
	void			GetMapButtonPosition(int index, int* x, int* y);
	int				GetMapLevel(int index);
	int				GetMapLevelMax(int index);
#endif
protected:
