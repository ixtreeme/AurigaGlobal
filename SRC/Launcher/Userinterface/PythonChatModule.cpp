...
PyObject * chatGetLinkFromHyperlink(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (0 == results[0].compare("item")) {
		...
		if (CItemManager::Instance().GetItemDataPointer(htoi(results[1].c_str()), &pItemData)) {
			...
			DWORD transmute = 0;
			if (results.size() > 6)
				transmute = htoi(results[6].c_str());

			len = _snprintf(itemlink, sizeof(itemlink), "item:%x:%x:%x:%x:%x:%x",
...

			{
...
#endif
				{
					...
					len += _snprintf(itemlink + len, sizeof(itemlink) - len, ":%x:%lld", type, atoi(results[i+1].c_str()));
				}
			}
		...
		char buf[1024] = { 0 };
		_snprintf(buf, sizeof(buf), "|cffc9c0f1|H|h%s|h|r", results[1].c_str());
		return Py_BuildValue("s", buf);
	}
#endif
