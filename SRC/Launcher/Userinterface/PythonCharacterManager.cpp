...
void CPythonCharacterManager::Update()
{
	...
	while (m_kAliveInstMap.end() != i)
	{
		...
		if (pkInstMain)
		{
			int nDistance = static_cast<int>(pkInstEach->NEW_GetDistanceFromDestInstance(*pkInstMain));
			if (nDistance > CHAR_STAGE_VIEW_BOUND + 10)
			{
				//uint32_t dwVID = pkInstEach->GetVirtualID();
				//CPythonTextTail::Instance().DeleteCharacterTextTail(dwVID);
				//CPythonBackground::Instance().DeleteSpecialEffect(dwVID);
...
CInstanceBase * CPythonCharacterManager::RegisterInstance(uint32_t VirtualID)
{
	...
	//TraceError("RegisterInstance: Registered character VID=%u\n", VirtualID);
	return (pCharacterInstance);
}

#ifdef ENABLE_MOUNT_COUNT_ABOVE_CHAR_RAZOR93
void CPythonCharacterManager::StorePendingMountCount(uint32_t dwVID, const std::string& szText)
{
	m_mapPendingMountCounts[dwVID] = szText;
}

void CPythonCharacterManager::ApplyPendingMountCountIfExists(CInstanceBase* pInstance)
{
	if (!pInstance)
		return;

	uint32_t dwVID = pInstance->GetVirtualID();
	auto it = m_mapPendingMountCounts.find(dwVID);
	if (it != m_mapPendingMountCounts.end())
	{
		pInstance->SetMountCountOverhead(it->second.c_str());
		m_mapPendingMountCounts.erase(it);
	}
}
#endif
...
#ifdef MEMORY_LEAK_FIX_RAZRO93
	CPythonTextTail::Instance().DeleteCharacterTextTail(dwDelVID);
#endif
