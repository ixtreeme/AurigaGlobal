...
void CPythonGuild::SetGuildTrophies(int iTrophies)
{
	m_GuildInfo.trophies = iTrophies;
}
#endif
...
void CPythonGuild::RegisterGuildName(uint32_t dwID, const char * c_szName)
{
	...
	RegisterGuildName(dwID, c_szName, 0);
#else
...
#ifdef ENABLE_GUILD_RENEWAL_BY_RAZOR93
void CPythonGuild::RegisterGuildName(uint32_t dwID, const char * c_szName, uint8_t byLevel)
{
	// Update name map (insert won't overwrite)
	m_GuildNameMap[dwID] = std::string(c_szName);
	m_GuildLevelMap[dwID] = byLevel;
}
#endif
...
int CPythonGuild::GetGuildLevel(uint32_t dwGuildID)
{
	if (dwGuildID == m_GuildInfo.dwGuildID)
		return m_GuildInfo.dwGuildLevel;

#ifdef ENABLE_GUILD_RENEWAL_BY_RAZOR93
	auto it = m_GuildLevelMap.find(dwGuildID);
	if (it != m_GuildLevelMap.end())
		return it->second;
#endif
	return 0;
}
...
#ifdef ADVANCED_GUILD_INFO
int CPythonGuild::GetGuildTrohpies()
{
	return m_GuildInfo.trophies;
}
int CPythonGuild::GetGuildWin()
{
	return m_GuildInfo.win;
}
int CPythonGuild::GetGuildLoss()
{
	return m_GuildInfo.loss;
}
int CPythonGuild::GetGuildDraw()
{
	return m_GuildInfo.draw;
}
#endif
...
void CPythonGuild::__CalculateLevelAverage()
{
	...
	for (auto itor = m_GuildMemberDataVector.begin(); itor != m_GuildMemberDataVector.end(); ++itor)
	{
...
#ifdef ADVANCED_GUILD_INFO
PyObject* guildGetGuildTrophies(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonGuild::Instance().GetGuildTrohpies());
}
PyObject* guildGetGuildStats(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("iii", CPythonGuild::Instance().GetGuildWin(), CPythonGuild::Instance().GetGuildLoss(), CPythonGuild::Instance().GetGuildDraw());
}
#endif
...
#ifdef ADVANCED_GUILD_INFO
		{ "GetGuildTrophies",				guildGetGuildTrophies,				METH_VARARGS },
		{ "GetGuildStats",					guildGetGuildStats,					METH_VARARGS },
#endif
