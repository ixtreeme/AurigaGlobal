...
bool CPythonSkillPet::RegisterSkillPet(const char * c_szFileName)
{
	...
	for (uint32_t i = 0; i < textFileLoader.GetLineCount()-1; ++i)
	{
		...
		/*
		std::map<uint32_t, TSkillDataPet>::iterator it = m_SkillDataPetMap.find(1);
...
CPythonSkillPet::CPythonSkillPet()
{
	...
	*/
	...
	petslot[3] = 0;
#endif
	m_SkillPetTypeIndexMap.insert(std::map<std::string, uint32_t>::value_type("EMPTY", SKILLPET_TYPE_NONE));
	m_SkillPetTypeIndexMap.insert(std::map<std::string, uint32_t>::value_type("PASSIVE", SKILLPET_TYPE_PASSIVE));
	m_SkillPetTypeIndexMap.insert(std::map<std::string, uint32_t>::value_type("AUTO", SKILLPET_TYPE_AUTO));
...
{
	const auto it = m_SkillDataPetMap.find(dwSkillIndex);
...
PyObject * petskillGetIconImage(PyObject * poSelf, PyObject * poArgs)
{
	...
	return Py_BuildException("Could not properly register window (PetSkill)");
}
...
static PyObject* petskillGetEmptySkill(PyObject* poSelf, PyObject* poArgs)
{
	char szIconFileNameHeader[65];
	_snprintf(szIconFileNameHeader, sizeof(szIconFileNameHeader), "d:/ymir work/ui/pet/skill_button/skill_enable_button.sub");

	auto noskillbtn = static_cast<CGraphicImage*>(CResourceManager::Instance().GetResourcePointer(szIconFileNameHeader));
	if (!noskillbtn)
		Py_RETURN_NONE;

	return PyCapsule_New(noskillbtn, nullptr, nullptr);
}
