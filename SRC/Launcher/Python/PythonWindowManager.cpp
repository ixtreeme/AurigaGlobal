...
namespace UI
{
	...
	CWindowManager::CWindowManager()
		: m_lWidth(0), m_lHeight(0),
		  ...
		  m_iIgnoreEndTime(0), m_poMouseHandler(nullptr), m_bAttachingFlag(FALSE),
		  m_dwAttachingType(0),
		  m_dwAttachingIndex(0), m_dwAttachingSlotNumber(0), m_byAttachingIconWidth(0), m_byAttachingIconHeight(0), m_pActiveWindow(nullptr), m_pLockWindow(nullptr), m_pPointWindow(nullptr),
		  m_pLeftCaptureWindow(nullptr),
		  m_pRightCaptureWindow(nullptr), m_pMiddleCaptureWindow(nullptr)
	{
	...
	{
		assert(m_LayerWindowMap.contains(c_szLayer));
	...
	void CWindowManager::NotifyDestroyWindow(CWindow * pWindow)
	{
		...
		for (auto itor = m_KeyCaptureWindowMap.begin(); itor != m_KeyCaptureWindowMap.end();)
		{
...
//		ResetCapture();
		...
		if (m_pLeftCaptureWindow)
		{
			...
			if (pWin->IsFlag(CWindow::FLAG_MOVABLE))
			{
				...
				if (pWin->IsFlag(CWindow::FLAG_LIMIT))
				{
					...
					limitRect.right = m_lWidth - (int32_t)limitRect.right;
					limitRect.bottom = m_lHeight - (int32_t)limitRect.bottom;

					if (x < (int32_t)limitRect.left)
						x = (int32_t)limitRect.left;
					else if (x + pWin->GetWidth() >= (int32_t)limitRect.right)
						x = (int32_t)limitRect.right - pWin->GetWidth();

					if (y < (int32_t)limitRect.top)
						y = (int32_t)limitRect.top;
					else if (y + pWin->GetHeight() >= (int32_t)limitRect.bottom)
	...
	void CWindowManager::RunMouseRightButtonUp(int32_t x, int32_t y)
	{
		...
		if (CWindow * pWin = GetPointWindow())
	...
	void CWindowManager::RunKeyDown(int vkey)
	{
		...
		if (pKeyCaptureWindow && std::find(m_ReserveDeleteWindowList.begin(), m_ReserveDeleteWindowList.end(), pKeyCaptureWindow) == m_ReserveDeleteWindowList.end())
		{
	...
	{
		if (const auto itor = m_KeyCaptureWindowMap.find(vkey); m_KeyCaptureWindowMap.end() != itor)
		{
