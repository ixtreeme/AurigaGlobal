...
bool PyTuple_GetLong(PyObject* poArgs, int pos, int32_t* ret)
{
	if (pos >= PyTuple_Size(poArgs))
		return false;
...
bool PyTuple_GetIntPtr(PyObject* poArgs, int pos, intptr_t* ret)
{
	if (pos >= PyTuple_Size(poArgs))
		return false;
	...
	*ret = (intptr_t)PyLong_AsLongLong(poItem);
	assert((*ret != -1) || !PyErr_Occurred());
	return (*ret != -1) || !PyErr_Occurred();
}

bool PyTuple_GetUnsignedLong(PyObject* poArgs, int pos, uint32_t* ret)
{
	if (pos >= PyTuple_Size(poArgs))
		return false;
...
bool PyTuple_GetUnsignedInteger(PyObject* poArgs, int pos, UINT* ret)
{
	if (pos >= PyTuple_Size(poArgs))
		return false;
...
bool PyTuple_GetBoolean(PyObject* poArgs, int pos, bool* ret)
{
	...
	*ret = PyLong_AsLongLong(poItem) ? true : false;
	return true;
}
...
bool PyCallClassMemberFunc(PyObject* poClass, const char* c_szFunc, PyObject* poArgs, bool* pisRet)
{
	...
	if (PyNumber_Check(poRet))
		*pisRet = (PyLong_AsLongLong(poRet) != 0);
	else
		*pisRet = true;
...
bool PyCallClassMemberFunc(PyObject* poClass, const char* c_szFunc, PyObject* poArgs, int32_t* plRetValue)
{
	...
	if (PyNumber_Check(poRet))
	{
		*plRetValue = PyLong_AsLongLong(poRet);
		Py_DECREF(poRet);
		return true;
	}
