...
static int32_t r_long(RFILE *p)
{
	...
	if (fp) {
		x = getc(fp);
		x |= (int32_t)getc(fp) << 8;
		x |= (int32_t)getc(fp) << 16;
		x |= (int32_t)getc(fp) << 24;
	}
	else {
		x = rs_byte(p);
		x |= (int32_t)rs_byte(p) << 8;
		x |= (int32_t)rs_byte(p) << 16;
		x |= (int32_t)rs_byte(p) << 24;
	}
#if SIZEOF_LONG > 4
...
{
	int32_t lo4 = r_long(p);
	int32_t hi4 = r_long(p);
#if SIZEOF_LONG > 4
	int32_t x = (hi4 << 32) | (lo4 & 0xFFFFFFFFL);
...
int32_t _PyMarshal_ReadLongFromFile(FILE *fp)
{
	RFILE rf;
	rf.fp = fp;
	return r_long(&rf);
