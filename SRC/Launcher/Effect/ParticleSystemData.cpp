...
bool CParticleSystemData::OnLoadScript(CTextFileLoader & rTextFileLoader)
{
...

	{
		...
		{
			float fTime = *it;
			float fR = GetTimeEventBlendValue(fTime, TimeEventR);
			float fG = GetTimeEventBlendValue(fTime, TimeEventG);
			float fB = GetTimeEventBlendValue(fTime, TimeEventB);
			float fA = GetTimeEventBlendValue(fTime, TimeEventA);
