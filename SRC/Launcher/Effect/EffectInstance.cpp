...
void CEffectInstance::UpdateSound()
{
	...
	{
		SoundEngine::Instance().UpdateSoundInstance(m_matGlobal._41, m_matGlobal._42, m_matGlobal._43, m_dwFrame, m_pSoundInstanceVector, false);
	}
	++m_dwFrame;
}
...
#endif
{
	...
	for (i = 0; i < pEffectData->GetParticleCount(); ++i)
	{
		...
		if (dwSkillColor != nullptr)
		{
			...
			{
				auto c = D3DXCOLOR(skill);
				
				for (auto it = prob->m_TimeEventColor.begin(); it != prob->m_TimeEventColor.end(); ++it)
				{
					const auto d = D3DXCOLOR(it->m_Value);
					c.a = d.a;
					it->m_Value = c;
				}
			}
			...
			{
				const CParticleProperty* prob_backup = pParticle->GetParticlePropertyBackupPointer();
				for (auto it = prob->m_TimeEventColor.begin(); it != prob->m_TimeEventColor.end(); ++it)
					it->m_Value = prob_backup->m_TimeEventColor[distance(prob->m_TimeEventColor.begin(), it)].m_Value;
...
void CEffectInstance::SetTimeScale(float fTimeScale)
{
	for (auto& elem : m_ElementInstanceVector)
	{
		if (elem)
			elem->SetTimeScale(fTimeScale);
	}
}
...
void CEffectInstance::Clear()
{
	...
	{
		for (auto& particleInstance : m_ParticleInstanceVector)
		{
			CParticleSystemInstance::Delete(particleInstance);
		}
	...
	{
		for (auto& meshInstance : m_MeshInstanceVector)
		{
			CEffectMeshInstance::Delete(meshInstance);
		}
	...
	{
		for (auto& lightInstance : m_LightInstanceVector)
		{
			CLightInstance::Delete(lightInstance);
		}
