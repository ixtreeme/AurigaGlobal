...
bool CParticleInstance::Update(float fElapsedTime, float fAngle)
{
	...
	if (fAngle)
	{
		...
		{
			D3DXQUATERNION q,qc;
			D3DXQuaternionRotationAxis(&q,&m_pParticleProperty->m_v3ZAxis, XMConvertToRadians(fAngle));
		...
		case BILLBOARD_TYPE_Y:
			{
				...
				//v3Up = D3DXVECTOR3(cosf(XMConvertToRadians(m_fRotation)),0.0f,-sinf(XMConvertToRadians(m_fRotation)));
				const D3DXVECTOR3 & c_rv3View = pCurrentCamera->GetView();
				if (v3Up.x * c_rv3View.y - v3Up.y * c_rv3View.x<0)
					v3Up*=-1;
				auto ix = D3DXVECTOR3(c_rv3View.x, c_rv3View.y, 0);
				D3DXVec3Cross(&v3Cross, &v3Up, &ix);
		...
		default:
			{
				...
				else
				{
					const D3DXVECTOR3 & c_rv3View = pCurrentCamera->GetView();
					D3DXQUATERNION q,qc;
					D3DXQuaternionRotationAxis(&q, &c_rv3View, XMConvertToRadians(m_fRotation));
					D3DXQuaternionConjugate(&qc, &q);
		...
		default:
			{
				...
				else
				{
					...
					D3DXMatrixRotationAxis(&matRotation, &c_rv3View, XMConvertToRadians(m_fRotation));
					D3DXVec3TransformCoord(&v3Up, &ix, &matRotation);
					D3DXVec3TransformCoord(&v3Cross, &c_rv3Up, &matRotation);
				}
			}
			break;
...
void CParticleInstance::__Initialize()
{
	...
	m_dcColor = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
