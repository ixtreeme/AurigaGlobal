...
void CParticleSystemInstance::CreateParticles(float fElapsedTime)
{
	...
	int iCreatingCount = static_cast<int>(fCreatingValue);
	m_fEmissionResidue = fCreatingValue - iCreatingCount;

	int icurEmissionCount = GetEmissionCount();
	int iMaxEmissionCount = static_cast<int>(m_pEmitterProperty->GetMaxEmissionCount());
	...
	for (int i = 0; i < iCreatingCount; ++i)
	{
		...
		{
			auto ix = pInstance->m_v3Position - v3TimePosition;
			D3DXVec3Normalize(&pInstance->m_v3Velocity, &ix);
		...
		else if (CEmitterProperty::EMITTER_ADVANCED_TYPE_OUTER == m_pEmitterProperty->GetEmitterAdvancedType())
		{
			...
			{
				auto ix2 = pInstance->m_v3Position - v3TimePosition;
				D3DXVec3Normalize(&pInstance->m_v3Velocity, &ix2);
				pInstance->m_v3Velocity *= 100.0f;
			}
		}
...
bool CParticleSystemInstance::OnUpdate(float fElapsedTime)
{
	...
	{
		auto ix = D3DXVECTOR3(0.0f, 0.0f, 1.0f);
		D3DXVec3TransformNormal(&m_pParticleProperty->m_v3ZAxis,&ix,mc_pmatLocal);
	}

	for (dwFrameIndex = 0; dwFrameIndex < dwFrameCount; dwFrameIndex++)
	{
		for (auto itor = m_ParticleInstanceListVector[dwFrameIndex].begin(); itor != m_ParticleInstanceListVector[dwFrameIndex].end();)
		{
	...
	for (dwFrameIndex = 0; dwFrameIndex < dwFrameCount; ++dwFrameIndex)
	{
		auto& targetVec = m_ParticleInstanceListVector[dwFrameIndex];
		auto& sourceVec = m_ParticleInstanceListVector[dwFrameIndex + dwFrameCount];

		targetVec.insert(targetVec.end(), std::make_move_iterator(sourceVec.begin()), std::make_move_iterator(sourceVec.end()));
...
namespace NParticleRenderer
{
	struct TwoSideRenderer
	{
		...
		explicit TwoSideRenderer(const D3DXMATRIX * pmat= nullptr)
			: pmat(pmat)
		{
		}
	...
	struct AttachRenderer
	{
		...
		explicit AttachRenderer(const D3DXMATRIX * pmat)
			: pmat(pmat)
		{
		}
...
{
	m_pData = static_cast<CParticleSystemData*>(pElement);
...
void CParticleSystemInstance::OnDestroy()
{
	...
	for (auto& imgInst : m_kVct_pkImgInst)
	{
		CGraphicImageInstance::Delete(imgInst);
	}
	m_kVct_pkImgInst.clear();
}

CParticleSystemInstance::CParticleSystemInstance() : m_dwCurrentEmissionCount(0), m_iLoopCount(0), m_fEmissionResidue(0), m_pData(0), m_pEmitterProperty(0), m_pParticleProperty(0)
{
	Initialize();
}
