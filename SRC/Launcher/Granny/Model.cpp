...
const CGrannyModel::TMeshNode* CGrannyModel::GetMeshNodeList(const CGrannyMesh::EType eMeshType,
	const CGrannyMaterial::EType eMtrlType) const
{
	return m_meshNodeLists[eMeshType][eMtrlType];
}

CGrannyMesh* CGrannyModel::GetMeshPointer(const int iMesh)
{
	assert(CheckMeshIndex(iMesh));
	assert(m_meshs != NULL);

	return m_meshs.get() + iMesh;
}

const CGrannyMesh* CGrannyModel::GetMeshPointer(const int iMesh) const
{
	assert(CheckMeshIndex(iMesh));
	assert(m_meshs != NULL);
...
bool CGrannyModel::LoadMeshs()
{
	...
	const granny_skeleton* pgrnSkeleton = m_pgrnModel->Skeleton;
	...
	constexpr int blendPNT2MeshNodeCount = 0;

	const int meshCount = GetMeshCount();

	m_meshs = std::make_unique<CGrannyMesh[]>(meshCount);
	...
	for (int m = 0; m < meshCount; ++m)
	{
		...
		const granny_int32x grni32xTypeCount = GrannyGetTotalTypeSize(pgrnMesh->PrimaryVertexData->VertexType) / sizeof(granny_data_type_definition);
		int i = 0;
		while (i < grni32xTypeCount)
		{
			if (nullptr == pgrnMesh->PrimaryVertexData->VertexType[i].Name || 0 == strlen(
				pgrnMesh->PrimaryVertexData->VertexType[i].Name))
			{
				++i;
				continue;
			}
	...
	m_meshNodes = std::make_unique<TMeshNode[]>(m_meshNodeCapacity);
...
BOOL CGrannyModel::CheckMeshIndex(const int iIndex) const
{
	if (iIndex < 0)
		return FALSE;
...
void CGrannyModel::AppendMeshNode(const CGrannyMesh::EType eMeshType, const CGrannyMaterial::EType eMtrlType, const int iMesh)
{
	...
	rMeshNode.pMesh = m_meshs.get() + iMesh;
...
bool CGrannyModel::CreateDeviceObjects() ////////////// FONTOS   for ciklust át nézni
{
	if (m_rigidVtxCount > 0)
	...
	{
		m_meshs[i];
		CGrannyMesh::RebuildTriGroupNodeList();
	}
