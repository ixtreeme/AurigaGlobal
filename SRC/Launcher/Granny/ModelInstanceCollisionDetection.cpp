                                     ...
                                     float*, float*, float* pt)
{
	...
	*pt = 100000000.0f;
	...
	constexpr float max = 10000000.0f;
...
void CGrannyModelInstance::GetBoundBox(D3DXVECTOR3* vtMin, D3DXVECTOR3* vtMax)
{
	...
	{
		const granny_mesh* pgrnMesh = m_pModel->GetGrannyModelPointer()->MeshBindings[m].Mesh;
		const int* boneIndices = __GetMeshBoneIndices(m);
...
bool CGrannyModelInstance::GetMeshMatrixPointer(const int iMesh, const D3DXMATRIX** c_ppMatrix) const
{
	if (!m_pgrnModelInstance)
		return false;
