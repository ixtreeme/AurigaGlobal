...
BOOL CGraphicThingInstance::GetBoundBox(uint32_t dwModelInstanceIndex, D3DXVECTOR3* vtMin, D3DXVECTOR3* vtMax)
{
	...
	D3DXVec3Scale(vtMin, vtMin, GetScale().x);
	D3DXVec3Scale(vtMax, vtMax, GetScale().x);
	return TRUE;
}
...
CGraphicThingInstance::CGraphicThingInstance() : m_bUpdated(0), m_fAverageSecondElapsed(0.0f), m_fDelay(0.0f), m_fLastLocalTime(0.0f), m_fLocalTime(0.0f), m_fRadius(0.0f), m_fSecondElapsed(0.0f)
{
	Initialize();
}
