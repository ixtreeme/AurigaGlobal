...
bool Sphere::RayIntersection(const Vector3d& rayOrigin, const Vector3d& rayDirection, Vector3d* intersect) const
{
	Vector3d EO = mCenter - rayOrigin;
	Vector3d V = rayDirection;
...
bool Sphere::RayIntersectionInFront(const Vector3d& rayOrigin, const Vector3d& rayDirection, Vector3d* intersect)
{
	Vector3d sect;
	bool hit = RayIntersection(rayOrigin, rayDirection, &sect);
...
void Sphere::Compute(const SphereInterface& source)
{
	...
	for (int j = 0; j < count; j++)
	{
		...
		{ 	/* this point is outside of current sphere */
			float old_to_p = sqrt(old_to_p_sq);
