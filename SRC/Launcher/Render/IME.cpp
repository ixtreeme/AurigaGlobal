...
protected:
	...
	public:
		...
		STDMETHODIMP BeginUIElement(uint32_t dwUIElementId, BOOL *pbShow);
		STDMETHODIMP UpdateUIElement(uint32_t dwUIElementId);
		STDMETHODIMP EndUIElement(uint32_t dwUIElementId);

		// ITfInputProcessorProfileActivationSink
		//   Notification for keyboard input locale change
		STDMETHODIMP OnActivated(uint32_t dwProfileType, LANGID langid, REFCLSID clsid, REFGUID catid,
...
bool CIME::Initialize(HWND hWnd)
{
	...
	if(GetSystemDirectoryA(szPath, MAX_PATH+1)) {
		...
		if(ms_hImm32Dll)
		{
			...
			BOOL (WINAPI* _ImmDisableTextFrameService)(uint32_t) = (BOOL (WINAPI*)(uint32_t))GetProcAddress(ms_hImm32Dll, "ImmDisableTextFrameService");
...
void CIME::AttributeProcess(HIMC hImc)
{
	...
	int32_t	attributeLen = ImmGetCompositionStringW(hImc, GCS_COMPATTR, &attribute, sizeof(attribute)) / sizeof(uint8_t);
...
void CIME::CandidateProcess(HIMC hImc)
{
	...
	if(dwCandidateLen > 0) {
		...
		if( GETLANG() == LANG_CHS ) {
			...
			{
				UINT uLen = lstrlenW((LPWSTR)((uint32_t)lpCandidateList + lpCandidateList->dwOffset[i])) + (3 - sizeof(WCHAR));
	    ...
	    memset(&ms_wszCandidate, 0, sizeof(ms_wszCandidate));
	    for(UINT i = iStartOfPage, j = 0; (uint32_t)i < lpCandidateList->dwCount && j < ms_dwCandidatePageSize; i++, j++) {
		...
		else
		{
			...
			switch(ms_adwId[0])
			{
				...
				case IMEID_CHT_VER44: // New ChanJie IME98b          : 4.4.x.x // WinXP
					p = *(uint8_t**)((uint8_t*)_ImmLockIMCC(lpIC->hPrivate) + 24);
					if (!p) break;
				...
				case IMEID_CHT_VER50: // 5.0.x.x // WinME
					p = *(uint8_t**)((uint8_t*)_ImmLockIMCC(lpIC->hPrivate) + 3 * 4);
					if(!p) break;
					p = *(uint8_t**)((uint8_t*)p + 1*4 + 5*4 + 4*2);
					if(!p) break;
				...
				case IMEID_CHS_VER53: // 5.3.x.x // SCIME2k or MSPY3 (w/OfficeXP and Whistler)
					p = *(uint8_t**)((uint8_t*)_ImmLockIMCC(lpIC->hPrivate) + 4);
					if(!p) break;
					p = *(uint8_t**)((uint8_t*)p + 1*4 + 5*4);
					if(!p) break;
				...
				case IMEID_CHS_VER41:
					{
						...
						p = *(uint8_t**)((uint8_t*)_ImmLockIMCC(lpIC->hPrivate) + nOffset * 4);
						if(!p) break;
...
uint32_t CIME::GetImeId( UINT uIndex )
{
	...
	uint32_t dwLang = ((uint32_t)hkl & 0xffff);
...
bool CIME::GetReadingWindowOrientation()
{
    ...
    if(!bHorizontalReading && (GETLANG() == LANG_CHT))
    {
		...
		int32_t lRc = RegOpenKeyExA(HKEY_CURRENT_USER, szRegPath, 0, KEY_READ, &hKey);
        if (lRc == ERROR_SUCCESS)
        {
			DWORD dwSize = sizeof(DWORD), dwMapping, dwType;
            lRc = RegQueryValueExA(hKey, "Keyboard Mapping", nullptr, &dwType, (uint8_t*)&dwMapping, &dwSize);
            if (lRc == ERROR_SUCCESS)
            {
                if ((dwVer <= MAKEIMEVERSION(5, 0) &&
                       ((uint8_t)dwMapping == 0x22 || (uint8_t)dwMapping == 0x23))
                     ||
                     ((dwVer == MAKEIMEVERSION(5, 1) || dwVer == MAKEIMEVERSION(5, 2)) &&
                       (uint8_t)dwMapping >= 0x22 && (uint8_t)dwMapping <= 0x24)
                  )
                {
...
void CIME::CheckToggleState()
{
	...
	bool bIme = ImmIsIME( ms_hklCurrent ) != 0
		&& ( ( 0xF0000000 & (uint32_t)ms_hklCurrent ) == 0xE0000000 ); // Hack to detect IME correctly. When IME is running as TIP, ImmIsIME() returns true for CHT US keyboard.
...
void CTsfUiLessMode::MakeCandidateStrings(ITfCandidateListUIElement* pcandidate)
{
	...
	for (UINT i = dwPageStart, j = 0; (uint32_t)i < CIME::ms_dwCandidateCount && j < CIME::ms_dwCandidatePageSize; i++, j++)
	{
