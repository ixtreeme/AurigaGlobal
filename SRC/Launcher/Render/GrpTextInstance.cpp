...
int CGraphicTextInstance::Hyperlink_GetText(char* buf, int len)
{
	...
	return WideCharToMultiByte(codePage, 0, gs_hyperlinkText.c_str(), (int)gs_hyperlinkText.length(), buf, len, nullptr, nullptr);
}

int CGraphicTextInstance::__DrawCharacter(CGraphicFontTexture* pFontTexture, WORD codePage, wchar_t text, uint32_t dwColor)
{
	...
	if (pInsCharInfo)
	{
		...
		m_textWidth += (WORD)pInsCharInfo->advance;
...
void CGraphicTextInstance::__GetTextPos(uint32_t index, float* x, float* y)
{
	...
	for (uint32_t i = m_startPos; i < index; ++i)
	{
...
void CGraphicTextInstance::Update()
{
	...
	CGraphicFontTexture::TCharacterInfomation* pSpaceInfo = pFontTexture->GetCharacterInfomation((WORD)dataCodePage, ' ');
			...
			for (int i = 0; i < wTextLen; )
			{
				...
				else
				{
					...
					else if (ret == TEXT_TAG_EMOJI_END)
					{
						...
						char retBuf[1024];
						int retLen = WideCharToMultiByte(GetDefaultCodePage(), 0, emojiBuffer.c_str(), (int)emojiBuffer.length(), retBuf, sizeof(retBuf) - 1, nullptr, nullptr);
						retBuf[retLen] = '\0';
						...
						if (CResourceManager::Instance().IsFileExist(szPath))
						{
							...
							if (CResourceManager::Instance().IsFileExist(stringPath.c_str())) {
								auto p_image = dynamic_cast<CGraphicImage*>(CResourceManager::Instance().GetResourcePointer(stringPath.c_str()));
								kEmoji.pInstance = CGraphicImageInstance::New();
								kEmoji.pInstance->SetImagePointer(p_image);
								m_emojiVector.push_back(kEmoji);
								memset(&kEmoji, 0, sizeof(SEmoji));

								for (int i = 0; i < (p_image->GetWidth()) / (pSpaceInfo->width - 1); ++i)
									x += __DrawCharacter(pFontTexture, dataCodePage, ' ', dwColor);
								if ((p_image->GetWidth()) % (pSpaceInfo->width - 1) > 1)
									x += __DrawCharacter(pFontTexture, dataCodePage, ' ', dwColor);
							}
						}
...
void CGraphicTextInstance::Render(RECT* pClipRect)
{
		...
		for (WORD i = m_startPos; i < m_endPos; ++i)
		{
			...
			if (m_bUseRenderingRect)
			{
				if (fCurLocalX < m_RenderingRect.left && !m_isMultiLine)
				{
					...
					fXRenderLeft = (m_RenderingRect.left - fCurLocalX) / fFontWidth;
				}
				else if (fCurLocalX + fFontWidth > m_RenderingRect.right && !m_isMultiLine)
				{
					...
					fXRenderRight = (fCurLocalX + fFontWidth - m_RenderingRect.right) / fFontWidth;
				}
			...
			akVertex[0].diffuse = akVertex[1].diffuse = akVertex[2].diffuse = akVertex[3].diffuse = m_dwColorInfoVector[
				i];
	...
	for (auto& p : verticesMap)
	{
		...
		{
			const auto batchCount = std::min<uint64_t>(LARGE_PDT_VERTEX_BUFFER_SIZE,
				l - f);
	...
	if (m_emojiVector.empty() == false)
	{
		for (auto& rEmo : m_emojiVector)
		{
			if (rEmo.pInstance)
			{
				...
				rEmo.pInstance->SetPosition(fStanX + rEmo.x + xoff, (fStanY + 7.0) - (rEmo.pInstance->GetHeight() / 2) + yoff);
...
void CGraphicTextInstance::SetValue(const char* c_szText, uint64_t len)
{
...
void CGraphicTextInstance::SetRenderingRect(float fLeft, float fTop, float fRight, float fBottom)
{
	...
	m_RenderingRect.left = (int)fWidth * (int)fLeft;
	m_RenderingRect.top = (int)fHeight * (int)fTop;
	m_RenderingRect.right = (int)fWidth * (int)fRight;
	m_RenderingRect.bottom = (int)fHeight * (int)fBottom;
}
