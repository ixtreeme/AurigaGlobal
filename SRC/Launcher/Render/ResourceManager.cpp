...
constexpr int32_t c_Deleting_Wait_Time = 30000;
constexpr int32_t c_DeletingCountPerFrame = 30;
constexpr int32_t c_Reference_Decrease_Wait_Time = 30000;
...
void CResourceManager::__DestroyResourceMap()
{
	...
	if (m_pResMap.size() == NULL)
	{
		Tracenf("CResourceManager::__DestroyResourceMap was NULL,delete was skipped!");
		return;
	}
...
void CResourceManager::DumpFileListToTextFile(const char* c_szFileName)
{
	...
	for (auto i = m_pResMap.begin(); i != m_pResMap.end(); ++i)
	{
		...
		if (pResource->IsType(CGraphicImage::Type()) && strnicmp(ext, ".sub", 4))
			filesize = static_cast<CGraphicImage*>(pResource)->GetWidth() * static_cast<CGraphicImage*>(pResource)->
				GetHeight() * 4;
		else
		{
		...
		data.KB = static_cast<float>(filesize) / static_cast<float>(1024);
...
void CResourceManager::Update()
{
	...
	{
		CResource* pResource = itor->first;
