...
constexpr float c_fDefaultResistance = 0.3f;
...
void CCamera::SetViewMatrix()
{
	...
	if (!std::isfinite(m_fDistance))
	{
		TraceError("Camera ERROR: invalid distance (NaN/INF) -> skip SetViewMatrix this frame");
		return;
	}
	...
	m_fPitch = D3DXVec3Dot(&m_v3Up, &ix_vec);// / D3DXVec2Length(&v2ViewYZ);
	if (m_fPitch >= 1)
		m_fPitch = 1;
	else if (m_fPitch <= -1)
		m_fPitch = -1;
...
void CCamera::RotateEyeAroundTarget(float fPitchDegree, float fRollDegree)
{
	...
	D3DXMatrixRotationAxis(&matRotPitch, &m_v3Cross, XMConvertToRadians(fPitchDegree));

	D3DXMatrixRotationZ(&matRotRoll, -XMConvertToRadians(fRollDegree));
...
//		return;
	...
	D3DXVec3TransformCoord(&m_v3Eye, &v3Temp, &matRot);
	m_v3Eye += v3Point;
	auto temp_up = v3Temp + m_v3Up;
	D3DXVec3TransformCoord(&m_v3Up, &temp_up, &matRot);
...
void CCamera::CalculateRoll()
{
	...
	auto vec1 = D3DXVECTOR2(0.0f, 1.0f);
	auto vec2 = D3DXVECTOR2(0.0f, 1.0f);

	float fDot = D3DXVec2Dot(&v2ViewXY, &vec1);
	if (fDot >= 1)
		fDot = 1;
	else if (fDot <= -1)
		fDot = -1;
	fDot = acosf(fDot);
	fDot *= (180.0f / D3DX_PI);
	float fCross = D3DXVec2CCW(&v2ViewXY, &vec2);
	if (0 > fCross)
	{
		fDot = -fDot;
	}
