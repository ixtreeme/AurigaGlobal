...
{
	auto pWnd = (CMSWindow *) GetWindowLongPtr(hWnd, GWLP_USERDATA);
...
bool CMSWindow::Create(const char* c_szName, int brush, uint32_t cs, uint32_t ws, HICON hIcon, int iCursorResource)
{
	...
	/*ws |= WS_POPUP;
	ws &= ~(WS_OVERLAPPEDWINDOW | WS_CAPTION | WS_THICKFRAME | WS_BORDER);*/
...
CMSWindow::CMSWindow(): m_rect(), m_isActive(false)
{
	m_hWnd = nullptr;
	m_isVisible = false;
}
