...
void CGraphicObjectInstance::SetRotation(float fRotation)
{
	...
	D3DXMatrixRotationZ(&m_mRotation, XMConvertToRadians(fRotation));
}
...
void CGraphicObjectInstance::SetRotation(float fYaw, float fPitch, float fRoll)
{
	...
	D3DXMatrixRotationYawPitchRoll(&m_mRotation, XMConvertToRadians(fYaw), XMConvertToRadians(fPitch), XMConvertToRadians(fRoll));
}
...
void CGraphicObjectInstance::SetScalePositionOld(float x, float y, float z)
{
...
{
	m_CullingHandle = nullptr;
	CGraphicObjectInstance::Initialize();
}
...
void CGraphicObjectInstance::UpdateBoundingSphere()
{
	if (m_CullingHandle)
	{
		...
		// Safety: ignore invalid results (NaN/INF/<=0). Keeping last valid sphere prevents random Hide() on camera rotate.
		if (!std::isfinite(radius) || radius <= 0.0f ||
			!std::isfinite((double)center.x) || !std::isfinite((double)center.y) || !std::isfinite((double)center.z))
			return;
...
{
	for(auto*& it : m_StaticCollisionInstanceVector)
	{
		if (it)
			it = nullptr;
	}
...
uint32_t CGraphicObjectInstance::GetCollisionInstanceCount() const
{
	return m_StaticCollisionInstanceVector.size();
}

CBaseCollisionInstance * CGraphicObjectInstance::GetCollisionInstanceData(uint32_t dwIndex) const
{
...
int CGraphicObjectInstance::GetPortal(uint32_t dwIndex) const
{
	if (dwIndex >= PORTAL_ID_MAX_NUM)
	{
		assert(dwIndex < PORTAL_ID_MAX_NUM);
