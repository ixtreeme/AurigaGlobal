	...
	if (IsSecurityMode())
	{
		...
		if (restSize > 0)
		{
			...
			if (decodeSize >= 8)
			{
				...
				/*int decodeDstSize = tea_decrypt((uint32_t *) (m_recvBuf + m_recvBufInputPos),
...
bool CNetworkStream::Connect(uint32_t dwAddr, uint16_t port, int limitSec)
{
	char szAddr[256];
	{
		uint8_t ip[4];
...
bool CNetworkStream::Connect(const char* c_szAddr, uint16_t port, int /*limitSec*/)
{
