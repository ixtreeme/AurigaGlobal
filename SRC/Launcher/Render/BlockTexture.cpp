...
void CBlockTexture::InvalidateRect(const RECT & c_rsrcRect)
{
	...
	{
		Tracef("InvalidateRect() - Strange rect: c_rsrcRect=(%d,%d,%d,%d), dstRect=(%d,%d,%d,%d)\n",
			c_rsrcRect.left, c_rsrcRect.top, c_rsrcRect.right, c_rsrcRect.bottom,
...
CBlockTexture::CBlockTexture(): m_rect(), m_clipRect(), m_bClipEnable(0), m_dwWidth(0), m_dwHeight(0)
{
	m_pDIB = nullptr;
