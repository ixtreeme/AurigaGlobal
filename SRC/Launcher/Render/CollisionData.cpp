...
{
	auto ix1 = s.v3Position - m_attribute.v3Position;
	auto ix2 = s.v3Position - s.v3LastPosition;

	if (D3DXVec3LengthSq(&ix1) >= (s.fRadius + m_attribute.fRadius) * (m_attribute.fRadius + s.fRadius))
		return {0.0f, 0.0f, 0.0f};
	D3DXVECTOR3 c;
	auto ix3 = D3DXVECTOR3(0.0f, 0.0f, 1.0f);
	D3DXVec3Cross(&c, &ix2, &ix3);
	auto ix_1 = s.v3Position - m_attribute.v3Position;
	
	float sum = -D3DXVec3Dot(&c, &ix_1);

	auto ix_2 = s.v3Position - m_attribute.v3Position;
	float mul = (s.fRadius + m_attribute.fRadius) * (s.fRadius + m_attribute.fRadius) - D3DXVec3LengthSq(&ix_2);
...
{
	const auto spherePos = s.v3Position - m_attribute.v3Position;
	const auto sphereLastPos = s.v3LastPosition - m_attribute.v3Position;

	const float distNow = D3DXVec3Dot(&m_attribute.v3Normal, &spherePos);
	const float distPrev = D3DXVec3Dot(&m_attribute.v3Normal, &sphereLastPos);

	const bool crossed = (distNow > 0.0f && distPrev < 0.0f) || (distNow < 0.0f && distPrev > 0.0f);
	const bool withinRadius = (distNow <= s.fRadius && distNow >= -s.fRadius);

	if (crossed || withinRadius)
	{
		const auto quadPos1 = s.v3Position - m_attribute.v3QuadPosition[0];
		const auto quadPos2 = s.v3Position - m_attribute.v3QuadPosition[3];

		const bool inside =
			D3DXVec3Dot(&quadPos1, &m_attribute.v3InsideVector[0]) > -s.fRadius &&
			D3DXVec3Dot(&quadPos1, &m_attribute.v3InsideVector[1]) > -s.fRadius &&
			D3DXVec3Dot(&quadPos2, &m_attribute.v3InsideVector[2]) > -s.fRadius &&
			D3DXVec3Dot(&quadPos2, &m_attribute.v3InsideVector[3]) > -s.fRadius;

		if (inside)
		{
			const float distCurrAbs = std::fabs(D3DXVec3Dot(&spherePos, &m_attribute.v3Normal));
			const float distPrevAbs = std::fabs(D3DXVec3Dot(&sphereLastPos, &m_attribute.v3Normal));

			if (distCurrAbs < distPrevAbs)
				return true;
		}
	}
...
D3DXVECTOR3 CPlaneCollisionInstance::OnGetCollisionMovementAdjust(const CDynamicSphereInstance & s) const
{
	...
	if (d>=-0.0001 && d<=0.0001)
		return {0.0f,0.0f,0.0f};
	const auto ix = s.v3Position - m_attribute.v3Position;
	const float t = -D3DXVec3Dot(&m_attribute.v3Normal, &ix) / d;
...
bool CCylinderCollisionInstance::CollideCylinderVSDynamicSphere(const TCylinderData& c_rattribute, const CDynamicSphereInstance& s) const
{
	...
	auto ix = oa - ob;
	return (D3DXVec3LengthSq(&ix) <= (c_rattribute.fRadius + s.fRadius) * (c_rattribute.fRadius + s.fRadius));
}
...
{
	auto v3Position = m_attribute.v3Position;
	v3Position.z = s.v3Position.z;

	const auto ixpos = s.v3Position - v3Position;
	const D3DXVECTOR3 ixdx{ 0.0f, 0.0f, 1.0f };

	const float radiusSum = s.fRadius + m_attribute.fRadius;
	if (D3DXVec3LengthSq(&ixpos) >= radiusSum * radiusSum)
		return { 0.0f, 0.0f, 0.0f };

	auto advance = s.v3Position - s.v3LastPosition;
	advance.z = 0.0f;

	D3DXVECTOR3 c;
	D3DXVec3Cross(&c, &advance, &ixdx);

	const float sum = -D3DXVec3Dot(&c, &ixpos);
	const float mul = radiusSum * radiusSum - D3DXVec3LengthSq(&ixpos);

	const float disc = sum * sum - 4.0f * mul;
	if (disc <= 0.0f)
		return { 0.0f, 0.0f, 0.0f };

	const float sq = std::sqrt(disc);
	const float t1 = (-sum - sq) * 0.5f;
	const float t2 = (-sum + sq) * 0.5f;

	const float t = (std::fabs(t1) <= std::fabs(t2)) ? t1 : t2;
	return (gc_fReduceMove * t) * c;
}
...
D3DXVECTOR3 CAABBCollisionInstance::OnGetCollisionMovementAdjust(const CDynamicSphereInstance & s) const
{
	...
	*/
	...
	auto ix = v3Temp - s.v3Position;
	if(D3DXVec3LengthSq(&ix) < s.fRadius * s.fRadius)
