...
HFONT CGraphicFontTexture::GetFont(WORD codePage)
{
	...
	if(const auto i = m_fontMap.find(codePage); i != m_fontMap.end())
	{
		hFont = i->second;
	}
	else
...
CGraphicFontTexture::TCharacterInfomation* CGraphicFontTexture::UpdateCharacterInfomation(TCharacterKey code)
{
	...
	auto lAdvance = ceilf( stABC.abcfA + stABC.abcfB + stABC.abcfC );
...
bool CGraphicFontTexture::CheckTextureIndex(uint32_t dwTexture) const
{
...
CGraphicImageTexture* CGraphicFontTexture::GetTexture(const DWORD dwTexture) const
{
	assert(CheckTextureIndex(dwTexture));
