...
#ifdef ENABLE_SCREEN_FREEZE_BUGFIX
#include "comdef.h"
#endif // ENABLE_SCREEN_FREEZE_BUGFIX
...
bool CScreen::RestoreDevice()
{
	...
	if (FAILED(hrTestCooperativeLevel))
	{
		...
		if (D3DERR_DEVICENOTRESET == hrTestCooperativeLevel)
		{
			...
			D3DDISPLAYMODE  rkD3DDMDesktop;

			if (FAILED(rkD3D.GetAdapterDisplayMode(D3DADAPTER_DEFAULT, &rkD3DDMDesktop)))
				return FALSE;
...
void CScreen::Show(HWND hWnd)
{
	...
	if (g_isBrowserMode)
	{
		RECT rcTop = { 0, 0, int32_t(ms_d3dPresentParameter.BackBufferWidth), int32_t(g_rcBrowser.top) };
		RECT rcBottom = { 0, int32_t(g_rcBrowser.bottom), int32_t(ms_d3dPresentParameter.BackBufferWidth), int32_t(ms_d3dPresentParameter.BackBufferHeight) };
		RECT rcLeft = { 0, int32_t(g_rcBrowser.top), int32_t(g_rcBrowser.left), int32_t(g_rcBrowser.bottom) };
		RECT rcRight = { int32_t(g_rcBrowser.right), int32_t(g_rcBrowser.top), int32_t(ms_d3dPresentParameter.BackBufferWidth), int32_t(g_rcBrowser.bottom) };
