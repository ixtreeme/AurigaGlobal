...
class CGraphicImageTexture : public CGraphicTexture
{
	public:
		...
		bool		Create(UINT width, UINT height, D3DFORMAT d3dFmt, uint32_t dwFilter = D3DX_FILTER_LINEAR);
		...
		bool		CreateFromMemoryFile(UINT bufSize, const void* c_pvBuf, D3DFORMAT d3dFmt, uint32_t dwFilter = D3DX_FILTER_LINEAR);
