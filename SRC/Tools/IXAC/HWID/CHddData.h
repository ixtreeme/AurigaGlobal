...
private:
	...
	void SetDiskData(int drive, uint32_t diskdata[256]);
	void SetDiskData(std::string model, std::string serial);

	char *flipAndCodeBytes(const char * str, int pos, int flip, char * buf);
	void ConvertToString(uint32_t diskdata[256], int firstIndex, int lastIndex, std::string& buf);
	void CleanWhitespaces(std::string& buf);

	bool DoIDENTIFY(HANDLE hPhysicalDriveIOCTL, PSENDCMDINPARAMS pSCIP, PSENDCMDOUTPARAMS pSCOP, uint8_t bIDCmd, uint8_t bDriveNum, PDWORD lpcbBytesReturned);

private:
	uint8_t IdOutCmd[sizeof(SENDCMDOUTPARAMS) + IDENTIFY_BUFFER_SIZE - 1];
...
#pragma pack()
	...
	typedef struct _GETVERSIONOUTPARAMS
	{
		...
		uint32_t fCapabilities; // Bit mask of driver capabilities.
		uint32_t dwReserved[4]; // For future use.
