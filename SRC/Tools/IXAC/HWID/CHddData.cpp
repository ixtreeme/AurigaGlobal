...
bool CHddData::ReadPhysicalDriveInNTWithAdminRights()
{
	...
	sprintf_s(driveName, sizeof(driveName), AY_OBFUSCATE("\\\\.\\PhysicalDrive%d"), drive);
	...
	if (!DoIDENTIFY(hPhysicalDriveIOCTL, &scip, (PSENDCMDOUTPARAMS)&IdOutCmd, (uint8_t)bIDCmd, (uint8_t)drive, &cbBytesReturned))
		return done;

	uint32_t diskdata[256];
	int ijk = 0;
...
bool CHddData::DoIDENTIFY(HANDLE hPhysicalDriveIOCTL, PSENDCMDINPARAMS pSCIP, PSENDCMDOUTPARAMS pSCOP, uint8_t bIDCmd, uint8_t bDriveNum, PDWORD lpcbBytesReturned) {
...
bool CHddData::ReadIdeDriveAsScsiDriveInNT()
{
	...
	for (controller = 0; controller < 16; controller++) //Required loop for this detection method dont remove <MartPwnS>
	{
		...
		strncpy_s((char *)p->Signature, 8 + 1, AY_OBFUSCATE("SCSIDISK"), 8 + 1);
...
void CHddData::SetDiskData(int drive, uint32_t diskdata[256])
{
...
void CHddData::ConvertToString(uint32_t diskdata[256], int firstIndex, int lastIndex, std::string& buf) {
	std::stringstream ss;
