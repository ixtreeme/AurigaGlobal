...
{
	if (const auto it = m_map_kGuild.find(GID); it != m_map_kGuild.end())
		return it->second;
...
{
	for(auto it_cont = m_WarMap.begin(); it_cont != m_WarMap.end(); ++it_cont)
		for(auto it = it_cont->second.begin(); it != it_cont->second.end(); ++it)
		{
	...
	for(auto it = m_DeclareMap.begin(); it != m_DeclareMap.end(); ++it)
	{
		FSendPeerWar(it->bType, GUILD_WAR_SEND_DECLARE, it->dwGuildID[0], it->dwGuildID[1]) (peer);
	}

	for(auto it = m_map_kWarReserve.begin(); it != m_map_kWarReserve.end(); ++it)
	{
...
bool CGuildManager::IsHalfWinLadderPoint(uint32_t dwGuildWinner, uint32_t dwGuildLoser)
{
	...
	if (const auto it = m_mapGuildWarEndTime[GID1].find(GID2); it != m_mapGuildWarEndTime[GID1].end() &&
...
void CGuildManager::StartWar(uint8_t bType, uint32_t GID1, uint32_t GID2, CGuildWarReserve * pkReserve)
{
	...
	m_pqOnWar.emplace(gw.tEndTime, gw.pElement);
}
...
void CGuildManager::UpdateScore(uint32_t dwGainGID, uint32_t dwOppGID, int iScoreDelta, int iBetScoreDelta)
{
	...
	if (const auto it = m_WarMap[GID1].find(GID2); it != m_WarMap[GID1].end())
	{
		TGuildWarPQElement * p = it->second.pElement;
...
void CGuildManager::DepositMoney(uint32_t dwGuild, int64_t iGold)
{
	if (iGold <= 0)
		return;
...
void CGuildManager::WithdrawMoney(CPeer* peer, uint32_t dwGuild, int64_t iGold)
{
	auto it = m_map_kGuild.find(dwGuild);
...
void CGuildManager::WithdrawMoneyReply(uint32_t dwGuild, uint8_t bGiveSuccess, int64_t iGold)
{
	auto it = m_map_kGuild.find(dwGuild);
...
bool CGuildManager::Bet(uint32_t dwID, const char * c_pszLogin, int64_t dwGold, uint32_t dwGuild)
{
...
bool CGuildWarReserve::Bet(const char * pszLogin, int64_t dwGold, uint32_t dwGuild)
{
	char szQuery[1024];
