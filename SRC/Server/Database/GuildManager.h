...
typedef struct SGuildWarInfo
{
    ...
    SGuildWarInfo() : tEndTime(0), pElement(nullptr), pkReserve(nullptr)
    {
    }
} TGuildWarInfo;
...
class CGuildWarReserve
{
	...
	bool	Bet(const char * pszLogin, int64_t dwGold, uint32_t dwGuild);
	void	Draw();
...
class CGuildManager : public singleton<CGuildManager>
{
	...
	int64_t		GetGuildGold(int64_t dwGuild);
	void	DepositMoney(uint32_t dwGuild, int64_t lGold);
	void	WithdrawMoney(CPeer* peer, uint32_t dwGuild, int64_t lGold);
	void	WithdrawMoneyReply(uint32_t dwGuild, uint8_t bGiveSuccess, int64_t lGold);

	void	MoneyChange(uint32_t dwGuild, int64_t dwGold);
	...
	bool	Bet(uint32_t dwID, const char * c_pszLogin, int64_t dwGold, uint32_t dwGuild);
