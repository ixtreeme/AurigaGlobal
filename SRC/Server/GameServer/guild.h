...
typedef struct SGuildMember
{
	...
	SGuildMember(entt::entity character, uint8_t grade, uint32_t offer_exp);
...
class CGuild
{
	public:
		...
		entt::entity	GetMasterCharacter();
		uint8_t		GetLevel() const		{ return m_data.level; }

		

#ifdef ENABLE_GUILD_RENEWAL_BY_RAZOR93
			bool		RenewalSetLevel(uint8_t level);
			void		RenewalSetLevelP2P(uint8_t level);
			template <typename Func>
			void ForEachOnlineMember(Func f)
			{
...
void		Reset() { m_data.power = m_data.max_power; }
		...
		void		NotifyGuildMaster(uint8_t type, uint32_t idx, const char * format, ...);
#endif
		...
		void	ChangeTrophies(bool bWinner, bool bDraw);
		int		GetTrophies() const {return m_data.trophies;}
		void	SaveTrophies();
		void	ResetAllStats();
