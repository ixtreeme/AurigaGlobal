...
class DESC_MANAGER : public singleton<DESC_MANAGER>
{
	public:
		...
		using PairedStringDWORD = std::pair<std::string, uint32_t>;
	...
	public:
...
#if defined(__IMPROVED_HANDSHAKE_PROCESS__)
		void AcceptHandshake(const char* c_szHost, uint32_t dwHandshakeTime = 0);
		...
		void SetIntruder(const char* c_szHost, uint32_t dwDelayHandshakeTime);
