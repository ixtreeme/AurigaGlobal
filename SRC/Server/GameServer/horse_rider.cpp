...
/*
   int iMinLevel;	// Ĺľ˝ÂÇŇ Ľö ŔÖ´Â ĂÖĽŇ ·ąş§
   int iNPCRace;
   int iMaxHealth;	// ¸»ŔÇ ĂÖ´ë ĂĽ·Â
   int iMaxStamina;	// ¸»ŔÇ ĂÖ´ë ˝şĹ×ąĚłĘ
   int iST;
   int iDX;
   int iHT;
   int iIQ;
...
{//	pclvl	lo		h	s	st	dx  ht  iq      dm  dmax vedelem
	...
	{ 25,	20101,	3,	4,	26,	1,	18,	9,	54,	43,	64,	20 },	// 1 (ĂĘ±Ţ)
	...
	{ 35,	20104,	18,	30,	40,	53,	27,	13,	69,	55,	82,	41 },	// 11 (Áß±Ţ)
	...
	{ 50,	20107,	35,	120,	53,	71,	36,	18,	84,	67,	100,	50 },	// 21 (°í±Ţ)
...
bool CHorseRider::ReviveHorse()
{
	...
	// 2005.03.24.ipkn.¸» »ě¸°ČÄ ´Ů˝Ă Á×´Â Çö»ó ĽöÁ¤
...
void CHorseRider::UpdateHorseDataByLogoff(uint32_t dwLogoffTime)
{
	...
	if (dwLogoffTime >= 12 * 60)
		UpdateHorseStamina(dwLogoffTime / 12 / 60, false); // ·Î±×żŔÇÁ 12şĐ´ç 1ľż Č¸şą
}
