...
typedef struct character_point_instant
{
	...
	// ¾Æ... ÁøÂ¥ ¿åÀ» ¾È ÇÒ·¡¾ß ¾È ÇÒ ¼ö°¡ ¾ø´Ù.
	// char´Â ÀÎº¥À» uint8_t array·Î grid¸¦ °ü¸®ÇÏ°í, exchange³ª cube´Â CGrid·Î grid¸¦ °ü¸®ÇÏ°í ¹¹³Ä ÀÌ°Å...
	// grid¸¦ ¸¸µé¾î ³ùÀ¸¸é grid¸¦ ¾²¶õ ¸»ÀÌ¾ß!!!
	// ¤µ¤² ¿ëÈ¥¼® ÀÎº¥À» ¶È°°ÀÌ µû¶ó¼­ ¸¸µç ³ªµµ Àß¸øÇß´Ù ¤Ð¤Ð
...
public:
	...
	bool			Goto(int32_t x, int32_t y);	// ¹Ù·Î ÀÌµ¿ ½ÃÅ°Áö ¾Ê°í ¸ñÇ¥ À§Ä¡·Î BLENDING ½ÃÅ²´Ù.
	void			Stop();

	bool			CanMove() const;		// ÀÌµ¿ÇÒ ¼ö ÀÖ´Â°¡?

	void			SyncPacket();
	bool			Sync(int32_t x, int32_t y);	// ½ÇÁ¦ ÀÌ ¸Þ¼Òµå·Î ÀÌµ¿ ÇÑ´Ù (°¢ Á¾ Á¶°Ç¿¡ ÀÇÇÑ ÀÌµ¿ ºÒ°¡°¡ ¾øÀ½)
	bool			Move(int32_t x, int32_t y);	// Á¶°ÇÀ» °Ë»çÇÏ°í Sync ¸Þ¼Òµå¸¦ ÅëÇØ ÀÌµ¿ ÇÑ´Ù.
	...
	void			SetLastAttacked(uint32_t time);	// ¸¶Áö¸·À¸·Î °ø°Ý¹ÞÀº ½Ã°£ ¹× À§Ä¡¸¦ ÀúÀåÇÔ
...
public:
	...
	void			GiveGold(int64_t iAmount);	// ÆÄÆ¼°¡ ÀÖÀ¸¸é ÆÄÆ¼ ºÐ¹è, ·Î±× µîÀÇ Ã³¸®
...
private:
	...
	// static const uint32_t		msc_dwDefaultChangeItemAttrCycle;	///< µðÆúÆ® ¾ÆÀÌÅÛ ¼Ó¼ºº¯°æ °¡´É ÁÖ±â
