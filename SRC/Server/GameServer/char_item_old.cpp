...
/* 	if (TItemPos(item->GetWindow(), item->GetCell()).IsBeltInventoryPosition())// @Razor93 GetWear(WEAR_BELT); ne legyen szukseges a wear_mount_costume hez
	...
	// #endif
			...
			case 70100: // Arany Zenbab – +5000 RP (korlátlan)

			{
				...
				uint32_t add_value = item->GetValue(0); // 5000

				UpdateAlignment(add_value);
				item->SetCount(item->GetCount() - 1);
			...
			case 39032:
			{
				...
				//const int cooldown = 3600; // 1 óra

				//if (now - GetLastFruitUse() < cooldown)
				//{
				//	int sec = cooldown - (now - GetLastFruitUse());
				//	ChatPacket(CHAT_TYPE_INFO, "cooldown: %d mp.", sec);
				//	return false;
				//}

				uint32_t add_value = item->GetValue(0); // 2000
				UpdateAlignment(add_value);
			...
			case 72100:
			{
				...
				//if (now - GetLastGoldFruitUse() < cooldown)
				//{
				//	int sec = cooldown - (now - GetLastGoldFruitUse());
				//	ChatPacket(CHAT_TYPE_INFO, "cooldown: %d mp.", sec);
				//	return false;
				//}

				uint32_t add_value = item->GetValue(0); // 10000
				UpdateAlignment(add_value);

				//SetLastGoldFruitUse(now);
				item->SetCount(item->GetCount() - 1);
			...
			case 80008:
			{
				...
				const uint64_t unitPrice = item->GetShopBuyPrice();  
				if (unitPrice == 0)
					break;
				...
				const uint64_t canAdd = (curGold + total > maxGold) ? (maxGold - curGold) : total;
				if (canAdd == 0)
					break;
...

{
	...
	else
	{
		if (item->IsDragonSoul())
		{
			...
			return false;
#ifdef ENABLE_EXTRA_INVENTORY
		// Csak befele (Inventory -> Extra) szigorítunk, hogy ha valami rossz bekerül, ki lehessen venni.
		if (DestCell.window_type == EXTRA_INVENTORY)
		{
			if (!item->IsExtraItem())
			{
				ChatPacket(CHAT_TYPE_INFO, "Extra inventory: Csak special itemeket tehetsz ide.");
				return false;
			}

			const uint8_t category = item->GetExtraCategory();

			const int begin = category * EXTRA_INVENTORY_CATEGORY_MAX_NUM;
			const int end = begin + EXTRA_INVENTORY_CATEGORY_MAX_NUM;

			if (DestCell.cell < begin || DestCell.cell >= end)
			{
				ChatPacket(CHAT_TYPE_INFO, "Extra inventory: Ez az item masik fulbe valo.");
				return false;
			}
		}
#endif
