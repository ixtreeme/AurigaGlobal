...
// DUAL-PATH: legacy only during migration window
	...
	{
#ifdef ENABLE_INGAME_DEBUG_RAZOR93
		ch->ChatPacket(CHAT_TYPE_INFO, "input_main.cpp::  instance().CreateDropItemVector(");//INGAME_DEBUG_RAZOR93
#endif
...
{
#ifdef ENABLE_INGAME_DEBUG_RAZOR93
	ch->ChatPacket(CHAT_TYPE_INFO, "input_main.cpp::  SendBlockChatInfo(");//INGAME_DEBUG_RAZOR93
#endif
	if (sec <= 0)
	{
...
{
#ifdef ENABLE_INGAME_DEBUG_RAZOR93
	ch->ChatPacket(CHAT_TYPE_INFO, "input_main.cpp::  bool SpamBlockCheck(LPCHARACTER ch, const char* const buf, const size_t buflen)(");//INGAME_DEBUG_RAZOR93
#endif
	...
	if (buflen > 1 && *buf == '/')
	{
		...
		// /shplink [extra szoveg...]  (alias: /shoplink)
		{
			...
			if (IsCmd("shplink") || IsCmd("shoplink"))
			{
				...
				if (!pkShop)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "Nincs nyitott offline boltod./ You don't have open shop.");
					return iExtraLen;
				}
...
#else
					ch->ChatPacket(CHAT_TYPE_INFO, "Nem megfelelo szint a kiabalashoz.");
#endif
					return iExtraLen;
				}
	...
	if (pinfo->type == CHAT_TYPE_SHOUT) {
		...
		do
		{
			...
			*q = '\0';
			...
			//rc->ChatPacketNew(CHAT_TYPE_INFO, 2181,
			//	"%s#%d#",
			//	ch->GetName(),            // %s (winner)
			...
			for (DESC_MANAGER::DESC_SET::const_iterator it = cset.begin(); it != cset.end(); ++it)
			{
				...
				d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, "%s", msg);
			}
#endif
...
{
#ifdef ENABLE_INGAME_DEBUG_RAZOR93
	ch->ChatPacket(CHAT_TYPE_INFO, "input_main.cpp::void CInputMain::Move(LPCHARACTER ch, const char * data)");//INGAME_DEBUG_RAZOR93
#endif
	if (!ch)
		return;
...
{
#ifdef ENABLE_INGAME_DEBUG_RAZOR93
	ch->ChatPacket(CHAT_TYPE_INFO, "input_main.cpp::void CInputMain::Attack(LPCHARACTER");//INGAME_DEBUG_RAZOR93
#endif
	if (nullptr == ch)
		return;
...
// DUAL-PATH: legacy only during migration window
	...
	if (p->ItemPos.IsBeltInventoryPosition())
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "You cannot place items from the Belt inventory into the safebox.");
...
//		}
	...
	else
	{
		...
		if (p->ItemPos.IsBeltInventoryPosition())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Nem helyezhetsz tárgyat közvetlenül a belt inventoryba.");
...
// DUAL-PATH: legacy only during migration window
	...
	// duplikáció tiltás + 18000 group szabály
	{
		...
		for (int i = 0; i < total; ++i)
		{
			...
			if (it->GetVnum() == vnum)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "This mount already added,you can't add two time!");
				return;
			}
		}
		...
		if (vnum >= 18000 && vnum <= 18149)
		{
			...
			for (int i = 0; i < total; ++i)
			{
				...
				if ((ov / 10) == group)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "You already have a belt of this type in your inventory.");
					return;
				}
			}
		}
...
{
#ifdef ENABLE_INGAME_DEBUG_RAZOR93
	ch->ChatPacket(CHAT_TYPE_INFO, "input_main.cpp:: int OfflineshopPacketCreateNewShop");//INGAME_DEBUG_RAZOR93
#endif
...
{
#ifdef ENABLE_INGAME_DEBUG_RAZOR93
	ch->ChatPacket(CHAT_TYPE_INFO, "input_main.cpp:: int OfflineshopPacketChangeShopName");//INGAME_DEBUG_RAZOR93
#endif
...
{
#ifdef ENABLE_INGAME_DEBUG_RAZOR93
	ch->ChatPacket(CHAT_TYPE_INFO, "input_main.cpp:: int OfflineshopPacketForceCloseShop");//INGAME_DEBUG_RAZOR93
#endif
	offlineshop::CShopManager& rManager = offlineshop::GetManager();
...
// DUAL-PATH: legacy only during migration window
...

	{
		if (ch->GetWheelDestiny())
		{
			...
			//{
			//	ch->ChatPacket(CHAT_TYPE_INFO, "Do not close now!!");
			//	return;
			//}
	...
	case TURN:
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Dungeonban nem tudsz pörgetni./You cannot in dungeon");
			return;
		}
		if (ch->GetWheelDestiny())
		{
			...
			if (ch->CountSpecifyItem(WHEEL_TICKET_VNUM) < 1)
			{
				...
				ch->ChatPacket(CHAT_TYPE_INFO, "You Dont have Battle Pass Ticket");
				return;
			}
