...
public:
    ...
    void Complete(int32_t mapIndex)
    {
        ...
        ForEachPcOnMap(mapIndex, [&](entt::entity ch){
                ...
                ecs::QuestSystem::SetFlag(ch, "valentine_dungeon.disconnect", 0);
                ecs::QuestSystem::SetFlag(ch, "valentine_dungeon.idx", 0);
                ecs::QuestSystem::SetFlag(ch, "valentine_dungeon.ch", 0);
                ecs::QuestSystem::SetFlag(ch, "valentine_dungeon.enter_time", 0);
                ecs::QuestSystem::SetFlag(ch, "valentine_dungeon.cooldown", now + kCooldownSeconds);
            });
...
EVENTFUNC(val_dungeon_check_event)
{
    ...
    if (DungeonSystem::GetFlag(d, "val_floor") != 2)
        return 0;
    ...
    if (metins == 0 && mobs == 0)
    {
        DungeonSystem::SetFlag(d, "val_f2_retry", 0);
        s_val.SpawnBoss(mapIndex);
        g_dispatcher.trigger(ecs::EvDungeonPrepare { CDungeonManager::instance().FindByMapIndex(mapIndex) });
        return 0;
    }

    int retry = DungeonSystem::GetFlag(d, "val_f2_retry");
    if (retry < 10)
    {
        DungeonSystem::SetFlag(d, "val_f2_retry", retry + 1);
...
EVENTFUNC(val_dungeon_to_floor2_event)
{
    ...
    if (DungeonSystem::GetFlag(d, "val_done") != 0)
        return 0;

    if (DungeonSystem::GetFlag(d, "val_floor") != 1)
        return 0;
...
void CValentineDungeon::OnPlayerDisconnect(entt::entity character)
{
    ...
    ecs::QuestSystem::SetFlag(character, "valentine_dungeon.disconnect", get_global_time() + kRejoinSeconds);
    ecs::QuestSystem::SetFlag(character, "valentine_dungeon.idx", idx);
    ecs::QuestSystem::SetFlag(character, "valentine_dungeon.ch", (int32_t)g_bChannel);
}
...
void CValentineDungeon::OnPlayerLogin(entt::entity character)
{
    ...
    if (DungeonSystem::GetFlag(d, kFlagCompleted) == 0)
    {
        ...
        if (floor == 1)
        {
            ChatToChar(character, "Valentine: Task #1: Destroy all stones. Remaining: %d", DungeonSystem::GetFlag(d, kFlagStep));
        }
        else if (floor == 2)
        {
...
bool CValentineDungeon::OnClickNpc(entt::entity character)
{
    ...
    // Rejoin flow
    const int32_t rejoinUntil = ecs::QuestSystem::GetFlag(character, "valentine_dungeon.disconnect");
    if (rejoinUntil > now)
    {
        const int32_t rejoinIdx = ecs::QuestSystem::GetFlag(character, "valentine_dungeon.idx");
        const int32_t rejoinCh = ecs::QuestSystem::GetFlag(character, "valentine_dungeon.ch");

        if (rejoinIdx >= kPrivateMin && rejoinIdx < kPrivateMax)
        {
            if (rejoinCh != 0 && rejoinCh != (int32_t)g_bChannel)
            {
                ecs::ChatSystem::Send(character, CHAT_TYPE_INFO, "You were in Valentine Dungeon on a different channel. Channel: %d", rejoinCh);
                return true;
            }
    ...
    {
        ecs::ChatSystem::Send(character, CHAT_TYPE_INFO, "Valentine: minimum level is %d.", kMinLevel);
        return true;
    }
    if (kMaxLevel > 0 && ecs::PointSystem::GetLevel(character) > kMaxLevel)
    {
        ecs::ChatSystem::Send(character, CHAT_TYPE_INFO, "Valentine: maximum level is %d.", kMaxLevel);
        return true;
    }

    // Cooldown
    const int32_t cdUntil = ecs::QuestSystem::GetFlag(character, "valentine_dungeon.cooldown");
    if (cdUntil > now)
    {
        const int32_t remain = cdUntil - now;
        ecs::ChatSystem::Send(character, CHAT_TYPE_INFO, "Valentine: you must wait %d seconds.", remain);
        return true;
    }
    ...
    {
        ecs::ChatSystem::Send(character, CHAT_TYPE_INFO, "Only the party leader can start Valentine Dungeon.");
        return true;
    }
...
if (!it.ok)
    ...
    const entt::entity d = CDungeonManager::instance().Create(kValOriginalMap);
    if (d == entt::null)
    {
        ecs::ChatSystem::Send(character, CHAT_TYPE_INFO, "Valentine: failed to create the dungeon.");
        return true;
    }
    ...
    auto applyMember = [&](entt::entity m){
            ...
            ecs::QuestSystem::SetFlag(m, "valentine_dungeon.idx", DungeonSystem::GetMapIndex(d));
            ecs::QuestSystem::SetFlag(m, "valentine_dungeon.ch", (int32_t)g_bChannel);
            ecs::QuestSystem::SetFlag(m, "valentine_dungeon.enter_time", now);
        };

    if (party == entt::null)
    {
        applyMember(character);
        DungeonSystem::Join_Coords(d, character, kEnterX, kEnterY, kValOriginalMap);
    }
    else
    {
        ForEachPcOnMap(ecs::PlayerRuntime::GetMapIndex(character), [&](entt::entity m){
    ...
    s_val.SchedulePrepare(DungeonSystem::GetMapIndex(d), kPrepareDelay);
    return true;
}
