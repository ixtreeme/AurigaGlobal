    ...
    public:
        ...
        void StartPrepare(int32_t mapIndex)
        {
            ...
            ForEachPcOnMap(mapIndex, [](LPCHARACTER pc) {
                if (pc) pc->ChatPacket(CHAT_TYPE_NOTICE, "[Pyramid] Destroy all metins!");
                });
        }
...
bool CPyramidDungeonRazor93::OnClickNpc(CHARACTER* ch)
{
    ...
    if (rejoinUntil > now)
    {
        ...
        if (IsPyramidDungeonMap(rejoinIdx))
        {
            ...
            {
                ch->ChatPacket(CHAT_TYPE_INFO, "You were in Pyramid Dungeon on a different channel: %d", rejoinCh);
                return true;
            }
    ...
    if (antiUntil > now)
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "Please wait %d seconds.", antiUntil - now);
        return true;
    }
    ...
    if (party && party->GetLeaderPID() != ch->GetPlayerID())
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "Only the party leader can start the Pyramid Dungeon.");
        return true;
    }
    ...
    if (!party)
    {
        ...
        if (lv < kMinLevel || lv > kMaxLevel)
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "Pyramid Dungeon requires level %d-%d.", kMinLevel, kMaxLevel);
            return true;
        }
    }
    else
    {
        ...
        if (!f.ok)
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "Pyramid Dungeon: %s has an invalid level (Lv%d). Required: %d-%d.",
    ...
    if (!party)
    {
        ...
        if (cdUntil > now)
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "Pyramid Dungeon is on cooldown (%d seconds).", cdUntil - now);
            return true;
        }
    }
    else
    {
        ...
        if (!f.ok)
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "Pyramid Dungeon: %s is on cooldown (%d seconds).",
    ...
    if (!party)
    {
        ...
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "You don't have the entry item.");
            return true;
        }
    }
    else
    {
        ...
        if (!f.ok)
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "%s doesn't have the entry item.", f.name ? f.name : "Someone");
            return true;
        }
    }
    ...
    if (!d)
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "Pyramid Dungeon: failed to create dungeon.");
        return true;
    }
...
void CPyramidDungeonRazor93::OnMobKilled(CHARACTER* killer, CHARACTER* victim)
{
    ...
    if (vnum == kMetinVnum)
    {
        ...
        ForEachPcOnMap(mapIdx, [s](LPCHARACTER pc) {
            if (pc) pc->ChatPacket(CHAT_TYPE_NOTICE, "[Pyramid] Metins remaining: %d", s);
            });
        ...
        if (s == 0)
        {
            ...
            ForEachPcOnMap(mapIdx, [](LPCHARACTER pc) {
                if (pc) pc->ChatPacket(CHAT_TYPE_NOTICE, "[Pyramid] The stone has appeared!");
                });
        }
        return;
    }
    ...
    if (vnum == kStoneVnum)
    {
        ...
        ForEachPcOnMap(mapIdx, [](LPCHARACTER pc) {
            if (pc) pc->ChatPacket(CHAT_TYPE_NOTICE, "[Pyramid] Kill all monsters!");
            });
        return;
    }
    ...
    if (vnum == 4621 || vnum == 4620 || vnum == 4618 || vnum == 4619)
    {
        ...
        if (d->CountMonster() == 0)
        {
            ...
            ForEachPcOnMap(mapIdx, [](LPCHARACTER pc) {
                if (pc) pc->ChatPacket(CHAT_TYPE_NOTICE, "-------- Kill the Boss! --------");
                });
        }
        return;
    }
    ...
    if (vnum == kBossVnum)
    {
        if (d->GetFlag(kFlagStep) == 0 && d->GetFlag(kFlagWasCompleted) == 0)
        {
            ...
            ForEachPcOnMap(mapIdx, [](LPCHARACTER pc) {
                if (pc) pc->ChatPacket(CHAT_TYPE_NOTICE, "[Pyramid] Dungeon completed!");
                });
        }
    }
}
