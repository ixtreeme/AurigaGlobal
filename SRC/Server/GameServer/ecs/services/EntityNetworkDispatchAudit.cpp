...
{
    // LPENTITY.4-architect.B.1.1.pre verifier.
    //
    // Iterates every entity that has both a legacy CHARACTER pointer and an
    // ECS Position component, and compares the two. While the inline getters
    // in entity.h still return m_pos.x/y/z, ch->GetX/GetY/GetZ samples the
    // legacy storage directly. Position is the dual-write twin populated by
    // SyncPositionComponents at every legacy SetXYZ call site.
    //
    // The B.1.1 first attempt (commits 9d27603 + fefce11) flipped the read
    // source to ECS Position prematurely without verifying every SetXYZ
    // caller actually pairs with SyncPositionComponents. The result was a
    // runtime regression even after the obvious MirrorLegacyMovement guard
    // fix - other dependencies on the legacy/ECS asymmetry survived.
    //
    // This sweep gives us proof that I1 (Position == m_pos) holds for every
    // live entity. Zero log output across a full WinTest session means the
    // dual-write contract is in fact tight and the read flip is safe.
    // Any [POSITION_READ_DRIFT] entry is a missing SyncPositionComponents
    // call site or a write path that bypasses SetXYZ.
    //
    // Cost: one full registry view iteration per call. Caller schedules
    // this at ~1Hz from main loop. With ~70K alive entities the iteration
    // is negligible.
    static thread_local std::unordered_map<uint64_t, uint32_t> s_lastLogTick;
    const uint32_t now = static_cast<uint32_t>(get_dword_time());

    auto view = reg.view<ecs::LegacyCharPtr, ecs::Position>();
    for (const entt::entity entity : view) {
        const auto& legacy = view.get<ecs::LegacyCharPtr>(entity);
        if (!legacy.ptr)
            continue;

        const auto& pos = view.get<ecs::Position>(entity);

        // GetX/GetY/GetZ currently return m_pos.x/y/z (inline accessors).
        // After Phase B.1.1 actual flip lands, these would read ECS Position
        // and the comparison becomes a tautology. The audit deletes at that
        // point.
        const int32_t legacyX = legacy.ptr->GetX();
        const int32_t legacyY = legacy.ptr->GetY();
        const int32_t legacyZ = legacy.ptr->GetZ();

        if (legacyX == pos.x && legacyY == pos.y && legacyZ == pos.z)
            continue;

        // Per-entity 30s rate limit (lesson from VIEWMAP_DRIFT spam in
        // 261e74d).
        const uint64_t key = static_cast<uint64_t>(entity);
        if (auto it = s_lastLogTick.find(key); it != s_lastLogTick.end()) {
            if (now - it->second < 30000u)
                continue;
        }
        s_lastLogTick[key] = now;

        LOG_WARN(
            "[POSITION_READ_DRIFT] entity={} legacy=({},{},{}) ecs=({},{},{}) name='{}'",
