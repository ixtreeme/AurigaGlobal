...
entt::entity CreateMobEntity(entt::registry& reg, const TMobTable& data, int x, int y, int mapIndex, uint32_t legacyVID)
{
    ...
    reg.emplace_or_replace<ecs::MobDataRef>(entity, mobProto, mobProto ? mobProto->m_table.dwVnum : 0);
...
entt::entity EntityFactory::CreatePC(entt::registry& reg, const TPlayerTable& data, LPDESC desc, uint32_t legacyVID)
{
    ...
    reg.emplace_or_replace<ecs::PartyMembership>(entity, now > 180000 ? now - 180000 : 0);
    reg.emplace_or_replace<ecs::GuildMembership>(entity, now > 60000 ? now - 60000 : 0);
    reg.emplace_or_replace<ecs::DungeonMembership>(entity, entt::null, 0, nullptr);
    reg.emplace_or_replace<ecs::MarriageState>(entity);
