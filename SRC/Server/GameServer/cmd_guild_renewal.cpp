...
ACMD(do_gr_pay_tax)
{
	...
	if (nums.empty())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Hasznalat: /gr_pay_tax <yang> <db0> <db1> <db2> <db3> <db4>");
		ch->ChatPacket(CHAT_TYPE_INFO, "Vagy: /gr_pay_tax <yang> <vnum0> <db0> ... <vnum4> <db4>");
		return;
	}
