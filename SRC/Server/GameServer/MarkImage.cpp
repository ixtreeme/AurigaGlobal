...
{
	LOG_INFO("GuildMarkImage: creating new file {}", c_szFileName);
...
bool CGuildMarkImage::Save(const char* c_szFileName)
{
	...
	{
		LOG_ERROR("GuildMarkImage::Save: stbi_write_tga failed for {}", c_szFileName);
		return false;
	}
...
bool CGuildMarkImage::Load(const char* c_szFileName)
{
	...
	if (!raw)
	{
		LOG_ERROR("GuildMarkImage::Load: {} cannot open file ({})", c_szFileName, stbi_failure_reason());
		return false;
	}

	if (w != WIDTH)
	{
		LOG_ERROR("GuildMarkImage::Load: {} width must be {} (got {})", c_szFileName, WIDTH, w);
		stbi_image_free(raw);
		return false;
	}
	...
	if (h != HEIGHT)
	{
		LOG_ERROR("GuildMarkImage::Load: {} height must be {} (got {})", c_szFileName, HEIGHT, h);
		stbi_image_free(raw);
		return false;
	}
...
bool CGuildMarkImage::SaveMark(uint32_t posMark, uint8_t* pbImage)
{
	...
	{
		LOG_ERROR("GuildMarkImage::SaveMark: invalid mark position {} (max {})", posMark, MARK_TOTAL_COUNT);
		return false;
	}
	...
	LOG_INFO("GuildMarkImage::SaveMark: pos={} block[{},{}] compSize={}", posMark, rowBlock, colBlock, m_aakBlock[rowBlock][colBlock].m_sizeCompBuf);
...
{
	Pixel apxBuf[SGuildMarkBlock::SIZE];
	LOG_INFO("GuildMarkImage::BuildAllBlocks");
...
void SGuildMarkBlock::CopyFrom(const uint8_t* pbCompBuf, uint32_t dwCompSize, uint32_t crc)
{
	...
	{
		LOG_ERROR("SGuildMarkBlock::CopyFrom: data too large ({} > {})", dwCompSize, MAX_COMP_SIZE);
		return;
	}
...
void SGuildMarkBlock::Compress(const Pixel* pxBuf)
{
	...
	if (compressedSize <= 0)
	{
		LOG_ERROR("SGuildMarkBlock::Compress: LZ4 compression failed");
		m_sizeCompBuf = 0;
		m_crc = 0;
		return;
	}
