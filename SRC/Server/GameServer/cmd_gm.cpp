...
//    if (!*a1)
//    {
//        ch->ChatPacket(CHAT_TYPE_INFO, "Hasznalat: /spawn_clon sourcePlayer targetPlayer [count]  |  /spawn_clon targetPlayer [count]");
//        return;
//    }
//
...
//    if (!source || !target)
//    {
//        ch->ChatPacket(CHAT_TYPE_INFO, "Hibas nev / celpont.");
//        return;
//    }
//
//    if (source->IsFakePlayer() || target->IsFakePlayer())
//    {
//        ch->ChatPacket(CHAT_TYPE_INFO, "Klonra nem lehet klont inditani.");
//        return;
//    }
//
//    if (!source->GetDesc() || !target->GetDesc())
//    {
//        ch->ChatPacket(CHAT_TYPE_INFO, "A celpont nem elerheto.");
//        return;
//    }
//
//    if (count <= 0) count = 1;
...
//    {
//        ch->ChatPacket(CHAT_TYPE_INFO, "Nem sikerult klont spawnolni.");
//        return;
//    }
//
//    ch->ChatPacket(CHAT_TYPE_INFO, "Klon spawn kesz: masolat=%s, target=%s, db=%d", source->GetName(), target->GetName(), count);
//}
//
//ACMD(do_p_clon)
//{
...
//        ch->ChatPacket(CHAT_TYPE_INFO, "Klonok torolve (target: te, map: %d)", mapIndex);
//        return;
//    }
//
...
//        ch->ChatPacket(CHAT_TYPE_INFO, "Osszes klon torolve ezen a mapon: %d", mapIndex);
//        return;
//    }
//
...
//    if (!target)
//    {
//        ch->ChatPacket(CHAT_TYPE_INFO, "Nincs ilyen player: %s", a1);
//        return;
//    }
//
//    CLostCastleDungeon::instance().PurgeTestClonesForTargetPID(target->GetPlayerID(), mapIndex);
//    ch->ChatPacket(CHAT_TYPE_INFO, "Klonok torolve (target: %s, map: %d)", target->GetName(), mapIndex);
//}
...
{
#ifdef TEXTS_IMPROVEMENT
	ch->ChatPacketNew(CHAT_TYPE_INFO, 737, "%d#%d", ecs::GetX(ch), ecs::GetY(ch));
#endif
...
ACMD(do_get_mob_count)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "MapIndex: %d MobCount %d", ecs::GetMapIndex(ch), f.nCount);
}
