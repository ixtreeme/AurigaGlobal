...
void LogManager::ItemLog(uint32_t dwPID, uint32_t x, uint32_t y, uint32_t dwItemID, const char * c_pszText, const char * c_pszHint, const char * c_pszIP, uint32_t dwVnum)
{
	...
	Query("INSERT IGNORE INTO log%s (type, time, who, x, y, what, how, hint, ip, vnum) VALUES('ITEM', NOW(), %u, %u, %u, %u, '%s', '%s', '%s', %u)",
...
{
	LOG_LEVEL_CHECK_N_RET(LOG_LEVEL_MIN);
	ItemLog(((ch)->GetPlayerID()), ((ch)->GetX()), ((ch)->GetY()), itemID, c_pszText, c_pszHint, ecs::PlayerRuntime::GetDesc(AIHelpers::EcsOf(ch)) ? ecs::PlayerRuntime::GetDesc(AIHelpers::EcsOf(ch))->GetHostName() : "", itemVnum);
}
...
void LogManager::CharLog(LPCHARACTER ch, uint32_t dw, const char * c_pszText, const char * c_pszHint)
{
	LOG_LEVEL_CHECK_N_RET(LOG_LEVEL_MIN);
	if (ch)
		CharLog(((ch)->GetPlayerID()), ((ch)->GetX()), ((ch)->GetY()), dw, c_pszText, c_pszHint, ecs::PlayerRuntime::GetDesc(AIHelpers::EcsOf(ch)) ? ecs::PlayerRuntime::GetDesc(AIHelpers::EcsOf(ch))->GetHostName() : "");
	else
...
void LogManager::MoneyLog(uint8_t type, uint32_t vnum, int64_t gold)
{
...
void LogManager::AttrTransferLog(uint32_t dwPID, uint32_t x, uint32_t y, uint32_t item_vnum)
{
	Query("INSERT INTO attr_transfer%s (pid, time, x, y, item_vnum) VALUES(%u, NOW(), %u, %u, %u)", get_table_postfix(), dwPID, x, y, item_vnum);
}
#endif

#ifdef ENABLE_NEW_OFFLINESHOP_LOGS
void LogManager::OfflineshopLog(const uint32_t dwOwnerID, const uint32_t dwItemID, const char* fmt, ...) {
