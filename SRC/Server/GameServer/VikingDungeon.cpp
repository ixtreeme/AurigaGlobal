...
namespace
{
    ...
    void SpawnFloor3ProtectorsForRemainingSlots(LPDUNGEON d)
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            if (protector)
                d->SetUnique(GetFloor3ProtectorKey(i), protector->GetVID());
        }
    }
    ...
    void ReplaceCompass(LPDUNGEON d, LPCHARACTER npc, uint32_t newVnum)
    {
        ...
        if (spawned)
            d->SetUnique("vk_compass", spawned->GetVID());
    ...
    public:
        ...
        long OnFloor1MainBoss(int32_t idx)
        {
            ...
            if (boss)
                d->SetUnique("vk_main_boss", boss->GetVID());
        ...
        long OnFloor2Transition(int32_t idx)
        {
            ...
            if (gate)
                d->SetUnique("vk_gate_2", gate->GetVID());
        ...
        long OnFinalSpawn(int32_t idx)
        {
            ...
            if (boss)
            {
                ...
                boss->SetMaxHP(hp);
                boss->SetHP(hp);
                d->SetUnique("vk_final_boss", boss->GetVID());
            }
            ScheduleFinalHp(idx);
...
void CVikingDungeon::OnPlayerLogin(CHARACTER* ch)
{
    ...
    {
        ch->SetQuestFlag(kQfDisconnect, 0);
        ch->ChatPacket(CHAT_TYPE_BIG_NOTICE, "Welcome back.");

        const int32_t limit = d->GetFlag(kFlagTimeLimit);
        if (d->GetFlag(kFlagCompleted) != 0)
        {
            ch->ChatPacket(CHAT_TYPE_BIG_NOTICE, "This instance will close soon. Pick up your drops quickly.");
        }
        else if (limit > get_global_time())
        {
            char tmp[64];
            FormatDuration(limit - get_global_time(), tmp, sizeof(tmp));
            ch->ChatPacket(CHAT_TYPE_BIG_NOTICE, "Time remaining: %s.", tmp);
        }
    }
...
bool CVikingDungeon::OnUseItem(CHARACTER* ch, CItem* item)
{
    ...
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "You cannot use this item while inside the dungeon.");
        return true;
    }
    ...
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "You can already enter the dungeon.");
        return true;
    }

    ch->RemoveSpecifyItem(kResetItemVnum, 1);
    ch->SetQuestFlag(kQfCooldown, 0);
    ch->ChatPacket(CHAT_TYPE_INFO, "Dungeon cooldown reset successfully.");
    return true;
}
...
bool CVikingDungeon::OnClickNpc(CHARACTER* ch, CHARACTER* npc)
{
    ...
    if (race == kRewardChestVnum)
    {
        ...
        ch->ChatPacket(CHAT_TYPE_INFO, "Reward received.");
        return true;
    }
    ...
    //{
    //    ch->ChatPacket(CHAT_TYPE_INFO, "You must be in the correct map to enter Frostbane Fortress.");
    //    return true;
    //}
    ...
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "The dungeon is currently blocked.");
        return true;
    }
    ...
    if (party)
    {
        ...
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "Only the party leader can start the dungeon.");
            return true;
        }

        if (party->GetNearMemberCount() != party->GetMemberCount())
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "Every party member must be online and near the NPC.");
            return true;
        }

        if ((int32_t)party->GetMemberCount() < kMinMembers)
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "Your party needs at least %d members.", kMinMembers);
            return true;
        }
    }
    ...
    if (!ok)
    {
        ...
        switch (bad)
        {
            ...
            case BAD_WARP:
                ...
                break;
            case BAD_ITEM:
                ch->ChatPacket(CHAT_TYPE_INFO, "%s does not have the required entry item.", badName ? badName : "A member");
                break;
            case BAD_COOLDOWN:
...
void CVikingDungeon::OnMobKilled(CHARACTER* killer, CHARACTER* victim)
{
    ...
    {
        const int killedSlot = FindFloor3StoneSlotByVid(d, victim->GetVID());
        if (killedSlot >= 0)
        ...
        if (boss)
            d->SetUnique("vk_floor3_boss", boss->GetVID());
