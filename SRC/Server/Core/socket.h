...
#define INVALID_SOCKET -1
    ...
    extern int		socket_read(socket_t desc, char* read_point, uint64_t space_left);
    extern int		socket_write(socket_t desc, const char *data, uint64_t length);

    extern int		socket_udp_read(socket_t desc, char * read_point, uint64_t space_left, struct sockaddr * from, socklen_t * fromlen);
