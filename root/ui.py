...
class Window(object):
	...
	def __del__(self):
		try:
			if hasattr(self, "hWnd") and self.hWnd and wndMgr:
...
if app.ENABLE_SEND_TARGET_INFO:
	class ListBoxExNew(Window):
		...
		def AppendItem(self, newItem):
			newHeight = newItem.GetHeight()
			if newHeight <= 0:
				import dbg
				dbg.TraceError("Invalid AppendItem height %d" % newHeight)
				return

			if self.stepSize <= 0:
				self.stepSize = newHeight

			if newHeight % self.stepSize != 0:
				self.__AdaptStepSize(newHeight)

			self.maxSteps += newHeight / self.stepSize
			newItem.SetParent(self)
			self.itemList.append(newItem)

		def __AdaptStepSize(self, newItemHeight):
			currentStep = self.stepSize
			recalculatedStep = self.__CalculateStepSize(newItemHeight)
			
			if recalculatedStep == currentStep or recalculatedStep <= 0:
				return
			
			viewHeight = self.__GetCurrentViewHeight(currentStep)
			basePixel = self.basePos * currentStep
			
			self.stepSize = recalculatedStep
			self.maxSteps = 0
			
			for item in self.itemList:
				self.maxSteps += item.GetHeight() / self.stepSize
			
			self.viewSteps = max(1, (viewHeight + self.stepSize - 1) / self.stepSize)
			
			if basePixel > 0:
				self.basePos = basePixel / self.stepSize
			else:
				self.basePos = 0
		...
		def __GetCurrentViewHeight(self, stepSize):
			height = self.GetHeight()
			if height <= 0:
				height = self.viewSteps * stepSize
			return height

		def __CalculateStepSize(self, newItemHeight):
			step = self.stepSize
