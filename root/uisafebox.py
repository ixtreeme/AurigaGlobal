...
class SafeboxWindow(ui.ScriptWindow):
	...
	def SelectEmptySlot(self, selectedSlotPos):
		selectedSlotPos = self.__LocalPosToGlobalPos(selectedSlotPos)# fix Razor93
		if mousemodule.mouseController.isAttached():
			...
			if player.SLOT_TYPE_SAFEBOX == attachedSlotType:
				net.SendSafeboxCheckoutPacket(selectedSlotPos)
				snd.PlaySound("sound/ui/drop.wav")

			else:
				attachedInvenType = player.SlotTypeToInvenType(attachedSlotType)

				# Ha nem inventory vagy extra inventory, ne csináljon semmit
				if attachedInvenType not in [player.INVENTORY, player.EXTRA_INVENTORY]:
					chat.AppendChat(chat.CHAT_TYPE_INFO, "You can't do this.")
					return

				# Küldjük a Checkin Packetet
