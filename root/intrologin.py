...
class LoginWindow(ui.ScriptWindow):
	...
	def __LoadScript(self, fileName):
		...
		try:
			liveBtn = self.GetChild("select_live_btn")
			testBtn = self.GetChild("select_test_btn")

			liveBtn.SetEvent(ui.__mem_func__(self.OnSelectLiveServer))
			testBtn.SetEvent(ui.__mem_func__(self.OnSelectTestServer))
	...
	if app.ENABLE_MULTI_LANGUAGE:
		...
		def OnClickLanguageButton(self, lang):
			if app.GetLocaleName() != lang:
				...
				app.ShellExecute("metin2client.exe")
	...
	def setChannel(self, channel):
		...
		# FONTOS: csak UI refresh itt, nem csatlakozás!
		self.__RefreshServerStateList()
	...
	def OnUpdate(self):
		if constinfo.restart == 1:
			app.ShellExecute("AurigaGlobal.exe")
			app.Exit()
		
		ServerStateChecker.Update()
	def OnSelectLiveServer(self):
		import serverinfo
		serverinfo.SERVER_MODE = "LIVE"
		serverinfo.UpdateServerConfig()

		### reset channel UI
		try:
			self.SetChannel(1)  # CH1 fix
			self.channelButtonList[0].SetUp()
		...
		self.PopupDisplayMessage("LIVE szerver kivalasztva!")

	def OnSelectTestServer(self):
		import serverinfo
		serverinfo.SERVER_MODE = "TEST"
		serverinfo.UpdateServerConfig()
		...
		self.PopupDisplayMessage("TEST szerver kivalasztva!")
	...
	def __OpenWebSite(self, idToOpen):
		...
		if idToOpen == 0:
			self.wndWeb.Open("https://auriga-global.eu/")
		elif idToOpen == 1:
			self.wndWeb.Open("https://auriga-global.eu/")
		elif idToOpen == 3:
			self.wndWeb.Open("https://auriga-global.eu/")
