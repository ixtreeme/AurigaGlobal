...
# dbg import + fallback, hogy biztosan legyen TraceError
try:
	import dbg
except ImportError:
	class dbg:
...
class AtlasWindow(ui.ScriptWindow):
	...
	class AtlasRenderer(ui.Window):
		...
		def OnRender(self):
			...
			# atlas csak saját window pozíciót használ, player pozíciót nem, de biztos, ami biztos:
			if math.isnan(fx) or math.isnan(fy) or math.isinf(fx) or math.isinf(fy):
				dbg.TraceError("MiniMap ERROR: AtlasRenderer.OnRender invalid window pos (x=%s,y=%s)\n" % (str(fx), str(fy)))
				return
...
class MiniMap(ui.ScriptWindow):
	...
	def __Initialize(self):
		...
		self.ChannelButton1 = 0
		self.ChannelButton2 = 0
		self.ChannelButton3 = 0
		self.ChannelButton4 = 0
	...
	def __LoadWindow(self):
		...
		try:
			...
			self.ChannelButton1 = self.GetChild("ChannelButton1")
			self.ChannelButton2 = self.GetChild("ChannelButton2")
			self.ChannelButton3 = self.GetChild("ChannelButton3")
			self.ChannelButton4 = self.GetChild("ChannelButton4")
			if app.ENABLE_DEFENSE_WAVE:
		...
		self.ChannelButton1.SetEvent(ui.__mem_func__(self.OnPressChannel1))
		self.ChannelButton2.SetEvent(ui.__mem_func__(self.OnPressChannel2))
		self.ChannelButton3.SetEvent(ui.__mem_func__(self.OnPressChannel3))
		self.ChannelButton4.SetEvent(ui.__mem_func__(self.OnPressChannel4))
	...
	def __ChangeChannel(self, channel_id):
		net.SendChatPacket("/channel " + str(channel_id))

	def OnPressChannel1(self):
		self.__ChangeChannel(1)

	def OnPressChannel2(self):
		self.__ChangeChannel(2)

	def OnPressChannel3(self):
		self.__ChangeChannel(3)

	def OnPressChannel4(self):
		self.__ChangeChannel(4)
	...
	# --- SAFE PLAYER POSITION + LOG ---
	def _GetSafePlayerPosition(self):
		try:
			(x, y, z) = player.GetMainCharacterPosition()
		except Exception as e:
			if not hasattr(self, "pos_error_logged"):
				self.pos_error_logged = True
				dbg.TraceError("MiniMap ERROR: GetMainCharacterPosition exception: %s\n" % (str(e)))
			return 0.0, 0.0, 0.0
		...
		if invalid:
			if not hasattr(self, "pos_error_logged"):
				self.pos_error_logged = True
				dbg.TraceError("MiniMap ERROR: Invalid player position (x=%s, y=%s)\n" % (str(x), str(y)))
			return 0.0, 0.0, 0.0
		else:
			if hasattr(self, "pos_error_logged"):
				dbg.TraceError("MiniMap: Position recovered (x=%d, y=%d)\n" % (x, y))
				del self.pos_error_logged

		return float(x), float(y), float(z)
	# --- END SAFE PLAYER POSITION ---
	...
	def OnUpdate(self):
		...
		# SAFE POSITION + NaN védelem (Update)
		(x, y, z) = self._GetSafePlayerPosition()
		miniMap.Update(x, y)
		
		# biztonságos integer koordináta a texthez
		safe_x = 0
		safe_y = 0
		try:
		...
		if not added:
			...
			self.infoValue2.SetText("(%d, %d)" % (safe_x, safe_y))
	...
	def OnRender(self):
		...
		# itt védekezünk a minimap C++ Render crash ellen is:
		try:
			(px, py, pz) = player.GetMainCharacterPosition()
			px = float(px)
			py = float(py)
		except Exception as e:
			# ha valamiért itt hibázik, inkább ne rendereljünk, minthogy crasheljen
			dbg.TraceError("MiniMap ERROR: OnRender GetMainCharacterPosition exception: %s\n" % str(e))
			return

		if math.isnan(px) or math.isinf(px) or math.isnan(py) or math.isinf(py):
			# logoljuk, de ne floodoljunk
			if not hasattr(self, "render_pos_error_logged"):
				self.render_pos_error_logged = True
				dbg.TraceError("MiniMap ERROR: OnRender blocked, invalid player position (x=%s, y=%s)\n" % (str(px), str(py)))
			return
		else:
			if hasattr(self, "render_pos_error_logged"):
				dbg.TraceError("MiniMap: OnRender resume, valid player position (x=%d, y=%d)\n" % (px, py))
				del self.render_pos_error_logged
		...
		if math.isnan(fx) or math.isinf(fx) or math.isnan(fy) or math.isinf(fy):
			dbg.TraceError("MiniMap ERROR: OnRender invalid window pos (x=%s,y=%s)\n" % (str(fx), str(fy)))
			return
	...
	def HideMiniMap(self):
		...
		self.ChannelButton1.Hide()
		self.ChannelButton2.Hide()
		self.ChannelButton3.Hide()
		self.ChannelButton4.Hide()
	...
	def ShowMiniMap(self):
		...
		self.ChannelButton1.Show()
		self.ChannelButton2.Show()
		self.ChannelButton3.Show()
		self.ChannelButton4.Show()
